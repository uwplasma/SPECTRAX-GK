...
The default linear operator uses the full drift/mirror couplings
(``operator="gx"``). A reduced energy-weighted closure is available via
``operator="energy"`` for simplified studies and cross-checks. The
``operator="full"`` alias selects the same full drift/mirror path for backward
