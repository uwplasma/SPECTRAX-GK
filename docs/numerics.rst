...
Putting the pieces together, the full linear operator is assembled from:
...
(``operator="full"``). A reduced energy-weighted closure is available via
``operator="energy"`` for simplified studies and cross-checks.
