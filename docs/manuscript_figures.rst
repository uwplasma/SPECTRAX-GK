...
.. list-table::
   ...
   * - Boundary-coefficient transport landscape
     ...
     - current artifact base: ``docs/_static/vmec_boundary_transport_landscape_rbc11_full.png`` with JSON/CSV companions. The scan perturbs the strict QA ``RBC(1,1)`` coefficient over ``[-75%, +75%]`` with 31 points and evaluates growth plus all explicit electrostatic quasilinear rules on the optimizer sample set, ``s=(0.45,0.64,0.78)``, ``alpha=(0,pi/4)``, and ``k_y rho_i=(0.10,0.30,0.50)``, at ``ntheta=16``, ``mboz=nboz=21``, ``Nl=1``, and ``Nm=2``. The bottom panel overlays the 19 strict coefficients that have passed true long-window post-transient nonlinear heat-flux ensemble gates over ``t=[1100,1500]``: the negative side, the zero-offset baseline, and the first three positive coefficients, ``+5%``, ``+10%``, and ``+15%``. Reduced/startup nonlinear-window diagnostics are deliberately excluded. The first ``-75%`` nonlinear point failed running-mean convergence over ``t=[350,700]`` and passed after continuation. The neighboring ``-70%`` point still failed the ``t=[700,1100]`` timestep-spread gate and passed after continuation to ``t=[1100,1500]``. The positive ``+5%`` point passes with mean ``Q_i≈10.8433``, mean-relative spread about ``1.70%``, and combined SEM/mean about ``1.91%``; the positive ``+10%`` point passes with mean ``Q_i≈9.6448``, mean-relative spread about ``2.27%``, and combined SEM/mean about ``1.84%``; and the positive ``+15%`` point passes with mean ``Q_i≈10.9084``, mean-relative spread about ``4.20%``, and combined SEM/mean about ``1.57%``. All positive-side accepted points use ``seed32``, ``seed33``, and ``dt=0.04`` variants after the pathological ``seed31`` base runs were excluded from the positive-side campaign. The remaining higher positive-side coefficients remain pending, so this figure is a launch/noise diagnostic and optimizer-design input, not a promoted nonlinear turbulent-flux optimization result.
