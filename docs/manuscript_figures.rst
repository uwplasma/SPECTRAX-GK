...
.. list-table::
   ...
   * - Nonlinear-gradient control-variate campaign
     ...
     - Closed as pre-run variance-reduction design; physics promotion still requires new long-window runs
     - current artifact bases: ``docs/_static/qa_ess_zbs10_rel7p5_variance_reduction_plan.png`` and ``docs/_static/qa_ess_zbs10_rel7p5_control_variate_campaign_plan.png`` with JSON/CSV/PDF companions. The rel7.5 ``ZBS(1,0)`` follow-up is local and response-resolved but variance limited. The midpoint common-mode control variate reduces apparent residual uncertainty to ``0.238``; after including the missing independent control-mean uncertainty, the launch plan calls for ``21`` matched plus/minus pairs, ``42`` new nonlinear runs, and projected combined uncertainty ``0.480``. This is a bounded campaign design, not a promoted nonlinear turbulent-flux gradient.
