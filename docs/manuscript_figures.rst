...
quasilinear heat-flux prediction is blocked at ``50.0%``; broad end-to-end
nonlinear turbulent-flux stellarator optimization is partial at ``72.9%``;
production nonlinear domain-decomposition speedup is partial at ``55.0%``; and
VMEC/Boozer holdout optimization is closed at ``100.0%``. These percentages are
