...
.. list-table::
   ...
   * - W7-X fluctuation-spectrum panel
     ...
     - Initial simulation-spectrum diagnostic closed; experimental transfer-function validation deferred
     - current artifact base: ``docs/_static/w7x_fluctuation_spectrum_panel.png`` with CSV/JSON/PDF companions. The panel is regenerated from the gated W7-X nonlinear ``t≈200`` NetCDF artifact, requires the corresponding nonlinear gate summary to pass before plotting, and records ``gate_index_include=false`` because it is a diagnostic figure rather than an additional release gate. It shows normalized ``k_y`` spectra for ``|\phi|^2``, ``W_\phi``, and ``|Q_i|``, the time-averaged ``k_x``-``k_y`` fluctuation-power map, the signed heat-flux spectrum, and a windowed temporal spectrum for the dominant nonzonal and zonal traces. This closes the reproducible simulation-spectrum panel needed for the current manuscript stack; a Doppler-reflectometry-facing density/zonal-frequency comparison remains deferred until the diagnostic transfer function and access model are encoded.
