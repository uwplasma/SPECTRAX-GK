...
.. list-table::
   ...
   * - Eigenfunction validation panel
     ...
     - Open
     - first shipped overlap artifact is ``docs/_static/kbm_eigenfunction_overlap_summary.png``; raw GX bundles now exist for KBM and W7-X under ``docs/_static/reference_modes/`` and the first bounded-cost raw overlay is ``docs/_static/kbm_eigenfunction_reference_overlay_ky0p3000.png``. That raw KBM overlay remains an open validation figure because the bounded extraction still shows low overlap and large relative ``L^2`` mismatch.
