...
.. list-table::
   ...
   * - Solver-objective geometry-gradient gate
     ...
     - Solver-ready linear-RHS gradient gate closed; mode-21 VMEC/Boozer state-to-solver eigenfrequency, quasilinear heat-flux-weight, and reduced nonlinear-window estimator gates closed for QH plus Li383; compact and VMEC/Boozer-perturbed nonlinear startup FD audits closed only as plumbing checks; multi-alpha reduced aggregate objective plumbing passes; selected optimized-equilibrium post-transient transport-window audit closed; earlier aspect-6 projected transport-gradient candidate audited as a negative long-window transfer result; nonlinear turbulence-gradient promotion remains open
