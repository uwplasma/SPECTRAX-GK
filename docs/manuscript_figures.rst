...
compact production nonlinear-window observable finite-difference audit, a
VMEC/Boozer-perturbed production nonlinear-window observable audit,
...
.. list-table::
   ...
   * - VMEC/Boozer nonlinear-window finite-difference audit
     - ``tools/build_vmec_boozer_nonlinear_window_fd_audit.py``
     - Closed for VMEC/Boozer geometry-perturbed observable path; local-gradient promotion open
     - current artifact base: ``docs/_static/vmec_boozer_nonlinear_window_fd_audit.png`` with CSV/JSON/PDF companions. It starts from the real mode-21 ``vmec_jax -> booz_xform_jax`` QH state bridge, writes perturbed sampled geometries to temporary NetCDF files, and runs compact production nonlinear windows at ``Rcos_mid_surface_m1 = base +/- 1e-5`` plus a repeated base point. The gate checks finite outputs, deterministic repeatability, bounded late-window coefficient of variation and trend, resolved geometry perturbation, and resolved central finite-difference response; response/base is about ``0.040``. The forward/backward response is asymmetric and not monotone, so this is an observable-path audit rather than a promoted local nonlinear gradient or optimized-equilibrium transport claim.
