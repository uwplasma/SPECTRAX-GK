...
.. list-table::
   ...
   * - VMEC/Boozer parity matrix
     - ``tools/build_vmec_boozer_parity_matrix.py`` and ``tools/build_vmec_boozer_qi_robustness.py``
     - Multi-equilibrium zero-beta equal-arc geometry gate closed at ``mboz=nboz=21`` using the selected QI ``ntheta=16`` floor
     - current artifact base: ``docs/_static/vmec_boozer_parity_matrix.png`` with JSON/CSV/PDF companions, plus ``docs/_static/vmec_boozer_qi_robustness.json``. It checks QH, QI, and shaped-tokamak fixtures against the imported VMEC/EIK runtime convention and rejects ``mboz,nboz < 21``. A later real rerun of the older QI ``ntheta=8`` point reached drift mismatch ``8.1879e-2`` against the unchanged ``8e-2`` release tolerance, so the robustness artifact blocks that floor and selects the complete ``ntheta=16`` row at about ``7.02e-2``. Input-only QI seeds without bundled ``wout`` references remain explicitly artifact-limited. This should be presented as a field-line geometry convention gate, not as a finite-beta transport-gradient validation.
