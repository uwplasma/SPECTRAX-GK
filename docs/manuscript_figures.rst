...
.. list-table::
   ...
   * - External-VMEC next-holdout runbook
     ...
     - current artifact base: ``docs/_static/external_vmec_next_holdout_runbook.png`` with CSV/JSON/PDF companions. It converts the holdout-gap report and external-VMEC linear screen into a fail-closed nonlinear launch plan. After the circular external-VMEC case closed at ``t=450`` and entered the holdout set, the shaped-tokamak pressure candidate was run to ``t=450`` at ``n48``/``n64`` but failed the high-grid convergence gate with common and least-window relative heat-flux differences about ``0.306``. The follow-on ``ITERModel_reference_nc`` same-family audit passes at ``t=450`` with common/least grid differences about ``0.056``/``0.055``, but it is scoped as reproducibility evidence because ITERModel is already consumed by the training reference. The refreshed linear screen includes the tracked CTH-like spectrum point with ``gamma≈0.0488`` at ``ky≈0.2857``. The current runbook selects that row only through an explicit modified-protocol contract: ``n48/n64/n80`` grids and ``t=150,250,350`` horizons after the earlier CTH-like ``32->48`` grid/window failure. The completed modified-protocol CTH-like harvest is a bounded convergence study, not an admitted holdout. The full ``n48/n64/n80`` gates fail at all horizons; the ``t=350`` high-grid figure ``docs/_static/external_vmec_cth_like_modified_t350_high_grid_convergence_gate.png`` records the best grid-refined candidate. The tracked high-grid JSON gates show that ``t=150`` has close grid means (``0.026``/``0.009``) but fails the common-window trend gate, while ``t=250`` and ``t=350`` pass their high-grid checks. The late high-grid horizon artifact ``docs/_static/external_vmec_cth_like_modified_late_high_grid_time_horizon_gate.png`` passes with common/least time-horizon changes about ``0.018``/``0.019``. The first ``n80`` seed/timestep replicate extraction on ``t=[250,350]`` passed all individual readiness gates but failed the strict ensemble-spread gate (``0.182 > 0.15``), demonstrating that the shorter window was not robust enough. The restart-continued ``t=[350,700]`` evidence, ``docs/_static/external_vmec_cth_like_modified_replicates_t700/replicate_ensemble_gate.png``, passes with mean heat flux about ``9.60``, mean-relative spread about ``0.041``, and combined SEM/mean about ``0.052``. This promotes CTH-like to a replicated high-grid candidate only; calibration use still requires an explicit high-grid admission decision because the full-grid gate remains failed. The same runbook still blocks unchanged failed-family replays, ``QI_stel_seed_3127`` remains near-marginal with ``gamma≈3.8e-3``, and ``basic_non_stellsym`` remains a VMEC flux-tube contract failure. This records the fail-closed admission requirements: ``split=holdout``, sufficiently unstable linear branch, passed grid/window convergence, post-transient transport window, and independence from the training reference. This is a planning artifact only; it does not promote an absolute-flux predictor.
