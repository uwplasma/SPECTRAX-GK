...
.. list-table::
   ...
   * - VMEC/Boozer nonlinear-gradient evidence
     - ``tools/artifacts/build_nonlinear_gradient_evidence.py``, ``tools/campaigns/write_vmec_state_mapping_campaign.py``, and ``tools/artifacts/build_vmec_state_to_input_mapping_response.py``
     - Closed for mapping and one scoped variance-reduced response; broad nonlinear-gradient promotion remains open
     - The measured symmetric ``RBC/ZBS`` state map is rank zero, while the ``LASYM=true`` ``RBS/ZBC`` response has rank ``2`` and condition number about ``1.02``. The short-bracket campaigns complete but fail response/locality gates. Separately, the 7.5% ``ZBS(1,0)`` independent 21-pair control-mean audit over ``t=[600,1100]`` passes with combined relative uncertainty ``0.311 < 0.5``. This is mapped-state and scoped estimator evidence, not a universal nonlinear turbulence-gradient result.
