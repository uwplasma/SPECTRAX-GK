...
.. list-table::
   ...
   * - True ``t=1500`` strict QA candidate nonlinear audits
     - ``tools/build_external_vmec_replicate_ensemble.py`` and ``tools/check_nonlinear_runtime_outputs.py``
     - Growth-objective and nonlinear-window-objective candidate triplets admitted as robust long-window signals; matched optimization-success and QL-calibration claims still open
     - current artifact bases: ``docs/_static/vmec_qa_t1500_replicates/growth_from_strict_baseline_t1500_ensemble_gate.png`` and ``docs/_static/vmec_qa_t1500_replicates/nonlinear_window_from_strict_baseline_t1500_ensemble_gate.png`` with JSON companions. Both use true full-horizon ``t=1500`` office runs, two seeds plus one timestep variant, and the strict ``t=[1100,1500]`` averaging window. The growth candidate passes with ``<Q_i> = 11.510``, mean relative spread ``0.0427``, and combined SEM/mean ``0.0124``; the nonlinear-window candidate passes with ``<Q_i> = 11.609``, mean relative spread ``0.0366``, and combined SEM/mean ``0.0177``. These panels demonstrate saturated trace robustness, but not relative transport reduction, because the matched strict QA baseline and quasilinear-objective triplets are still running.
   * - Inverse/UQ figure
