...
.. list-table::
   ...
   * - Quasilinear calibration audit
     ...
     - current artifact bases: ``docs/_static/quasilinear_cyclone_calibration_audit.png``, ``docs/_static/quasilinear_cyclone_miller_calibration_audit.png``, ``docs/_static/quasilinear_cyclone_miller_train_holdout.png``, ``docs/_static/quasilinear_hsx_train_holdout.png``, ``docs/_static/quasilinear_w7x_train_holdout.png``, the manuscript-facing combined panel ``docs/_static/quasilinear_stellarator_train_holdout.png``, and the input-provenance audit ``docs/_static/quasilinear_validated_calibration_inputs.png`` with JSON/PDF companions. The current one-constant train/holdout report fits the heat-flux scale on Cyclone and the external-VMEC ITERModel case, then scores seven held-out windows: Cyclone Miller, HSX, W7-X, D-shaped external VMEC, up-down asymmetric external VMEC, circular external VMEC, and the CTH-like external VMEC case admitted only under the explicit high-grid policy. It intentionally remains ``calibration_dataset`` with ``passed = false`` because held-out errors exceed the ``0.35`` gate; the current holdout mean relative error is about ``1.91``. HSX, W7-X, and the up-down asymmetric VMEC point remain useful negative transfer checks because the simple positive-growth mixing-length family predicts little or near-zero flux while the nonlinear windows are finite. The circular and CTH-like external-VMEC points are better predicted by the scaled one-constant diagnostic, but they do not rescue the aggregate absolute-flux gate. The input audit confirms that every current train/holdout nonlinear artifact maps to a passed nonlinear gate or to the scoped high-grid admission gate, while failed QH and older CTH-like feasibility pilots remain excluded. This closes the NetCDF/CSV/high-grid-admission calibration machinery and provenance gate but not a calibrated absolute-flux claim.
   * - Quasilinear saturation-rule sweep
     ...
     - current artifact base: ``docs/_static/quasilinear_saturation_rule_sweep.png`` with JSON companion. It fits one scalar on the two training cases, Cyclone and external-VMEC ITERModel, then scores the seven held-out windows now admitted for three simple rules: positive-growth mixing length, raw linear heat-flux weight, and an absolute-growth diagnostic. All three fail the held-out absolute-flux gate. The least-bad simple rule is positive-growth mixing length with holdout mean relative error about ``1.91``; raw linear weight is about ``2.44`` and the absolute-growth diagnostic is about ``2.97``. The panel also includes a training-mean null baseline with holdout mean relative error about ``1.10``. Its JSON ``promotion_gate`` has no accepted rules, so future calibrated rules must beat both the quasilinear baselines and this null baseline before being promoted. This supports the next saturation-model lane while preventing premature absolute quasilinear transport claims.
   ...
   * - Stellarator quasilinear usefulness summary
     ...
     - current artifact base: ``docs/_static/quasilinear_stellarator_usefulness.png`` with CSV/JSON/PDF companions. It compares admitted HSX and W7-X nonlinear heat-flux windows with the simple positive-growth mixing-length rule, the calibrated linear-weight rule, and the best ``spectral_envelope_ridge`` rank-screening candidate. The simple rule predicts zero for HSX and W7-X while the nonlinear windows are finite, so it is explicitly rejected as a stellarator absolute-flux proxy. The spectral-envelope candidate is much closer on those two admitted stellarator holdouts but is no longer accepted by the strict uncertainty/model-selection gate; the figure also records that QA is matched-nonlinear-audit-only and QH is excluded until grid/window convergence passes. This figure is a manuscript-facing usefulness/limitation summary, not a runtime/TOML absolute-flux predictor.
   * - Quasilinear screening and rank-correlation skill
     ...
     - current artifact base: ``docs/_static/quasilinear_screening_skill.png`` with CSV/JSON/PDF companions. It scores the current simple rules and candidate models by Spearman rank correlation, pairwise order accuracy, and mean relative error on the nine-case electrostatic portfolio. Only ``spectral_envelope_ridge`` passes the full-portfolio and held-out-only rank/correlation gates, with full Spearman about ``0.78`` and held-out Spearman about ``0.75``. The mean-error gate remains empty because the best held-out mean relative error is about ``0.355`` against the ``0.35`` gate, so the figure deliberately reports ``absolute promotion: none`` and points to additional independent nonlinear holdouts as the next requirement before screening promotion can be reconsidered.
   ...
   * - Quasilinear model-selection status
     ...
     - Model-selection lane demoted by the expanded CTH-like holdout; rank screening remains useful
     - current artifact base: ``docs/_static/quasilinear_model_selection_status.png`` with CSV/JSON companion. It consolidates the dataset-sufficiency gate, uncertainty/skill gate, and tracked train/holdout calibration reports into one claim-boundary panel. The required ``spectral_envelope_ridge`` candidate reaches leave-one-geometry-out mean relative error about ``0.377`` with prediction-interval coverage ``8/9``. It beats the calibrated linear-weight baseline but misses the ``0.35`` transport gate, so the model-selection artifact is ``passed=false`` with blockers ``dataset_sufficiency_passed``, ``candidate_uncertainty_passed``, ``required_candidate_accepted``, and ``required_candidate_transport_error``.
   * - Quasilinear holdout-gap report
     ...
     - current artifact base: ``docs/_static/quasilinear_holdout_gap_report.png`` with CSV/JSON/PDF companions. It keeps ``absolute_flux_promoted=false`` and records both ``absolute_flux_promotion_requirements`` and ``screening_promotion_requirements`` blocks. The current absolute train/holdout error is about ``1.91`` against the ``0.35`` gate, i.e. about ``5.45`` times too large, with Cyclone Miller as the worst admitted holdout. The full-portfolio and held-out-only rank/correlation gates pass for ``spectral_envelope_ridge``, but screening promotion is still blocked by the requirement for two additional independent passed holdouts. The CTH-like high-grid admission satisfies the external-VMEC-family and non-axisymmetric external-VMEC coverage requirements; the report still requires substantially better absolute-flux transport error before absolute-flux promotion can be reconsidered.
   * - External-VMEC next-holdout runbook
     ...
     - current artifact base: ``docs/_static/external_vmec_next_holdout_runbook.png`` with CSV/JSON/PDF companions. It converts the holdout-gap report and external-VMEC linear screen into a fail-closed nonlinear launch plan. After the circular external-VMEC case closed at ``t=450`` and entered the holdout set, the shaped-tokamak pressure candidate was run to ``t=450`` at ``n48``/``n64`` but failed the high-grid convergence gate with common and least-window relative heat-flux differences about ``0.306``. The follow-on ``ITERModel_reference_nc`` same-family audit passes at ``t=450`` with common/least grid differences about ``0.056``/``0.055``, but it is scoped as reproducibility evidence because ITERModel is already consumed by the training reference. The modified-protocol CTH-like harvest is now admitted only through ``tools/check_external_vmec_high_grid_admission.py``: the full ``n48/n64/n80`` t=350 sidecar fails only grid-difference metrics, the retained ``n64/n80`` high-grid gates pass at ``t=250``/``t=350``, the late time-horizon gate passes with common/least changes about ``0.018``/``0.019``, and the restart-continued ``n80`` seed/timestep ensemble passes on ``t=[350,700]`` with mean heat flux about ``9.60``, mean-relative spread about ``0.041``, and combined SEM/mean about ``0.052``. With CTH-like now admitted as a scoped high-grid holdout, the refreshed runbook selects the shaped-tokamak-pressure reference as the next independent modified-protocol repair: ``n64/n80/n96`` grids, ``t=450,650`` horizons, and the screened unstable branch ``gamma≈0.0473`` at ``ky≈0.4762``. The same runbook still blocks unchanged failed-family replays, ``QI_stel_seed_3127`` remains near-marginal with ``gamma≈3.8e-3``, and ``basic_non_stellsym`` remains a VMEC flux-tube contract failure. This records the fail-closed admission requirements: ``split=holdout``, sufficiently unstable linear branch, passed grid/window or high-grid-admission evidence, post-transient transport window, and independence from the training reference. This does not promote an absolute-flux predictor.
