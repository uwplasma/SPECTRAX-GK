...
.. list-table::
   ...
   * - Solver-objective geometry-gradient gate
     - ``tools/build_solver_objective_gradient_gate.py``, ``tools/build_vmec_boozer_solver_frequency_gradient_gate.py``, ``tools/build_vmec_boozer_quasilinear_gradient_gate.py``, and ``tools/build_vmec_boozer_gradient_holdout_matrix.py``
     - Solver-ready production linear-RHS gradient gate closed; mode-21 VMEC/Boozer state-to-solver eigenfrequency and quasilinear heat-flux-weight gates closed for QH plus Li383; nonlinear-window state-gradient promotion open
     - current artifact bases: ``docs/_static/solver_objective_gradient_gate.png``, ``docs/_static/vmec_boozer_solver_frequency_gradient_gate.png``, ``docs/_static/vmec_boozer_quasilinear_gradient_gate.png``, and ``docs/_static/vmec_boozer_gradient_holdout_matrix.png`` with CSV/JSON/PDF companions. The first differentiates actual electrostatic linear-RHS eigenpair observables with respect to solver-ready geometry arrays using the implicit left/right eigenpair system and checks ``gamma``, ``omega``, ``<k_perp^2>``, linear heat/particle-flux weights, and a mixing-length heat-flux proxy against nearest-branch central finite differences. The VMEC/Boozer frequency gate starts from a real ``vmec_jax`` state coefficient, maps through ``booz_xform_jax`` with ``mboz=nboz=21``, builds the SPECTRAX-GK linear RHS, and verifies the eigenfrequency gradient. The VMEC/Boozer quasilinear gate uses a richer ``Nl=2, Nm=3`` moment basis and checks ``gamma``, ``omega``, ``<k_perp^2>``, ``Q_i`` weight, and ``gamma Q_i/k_perp^2`` against finite differences. The multi-equilibrium matrix repeats the frequency and quasilinear gates on the tracked QH and Li383 fixtures and passes with maximum relative mismatch ``4.9e-3``. A surface-stencil path is available for memory-bounded diagnostics, but nonlinear-window heat-flux gradients are still a separate future gate.
   * - Inverse/UQ figure
