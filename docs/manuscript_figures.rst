...
.. list-table::
   ...
   * - Quasilinear saturation-rule sweep
     ...
     - current artifact base: ``docs/_static/quasilinear_saturation_rule_sweep.png`` with JSON/PDF companions. This is the pre-CTH simple-rule sweep: it fits one scalar on the two training cases, Cyclone and external-VMEC ITERModel, then scores the six held-out windows then present for three simple rules: positive-growth mixing length, raw linear heat-flux weight, and an absolute-growth diagnostic. All three fail the held-out absolute-flux gate. The least-bad simple rule is positive-growth mixing length with holdout mean relative error about ``2.11``; raw linear weight is about ``2.68`` and the absolute-growth diagnostic is about ``3.32``. The panel also includes a training-mean null baseline with holdout mean relative error about ``1.20``. Its JSON ``promotion_gate`` has no accepted rules, so future calibrated rules must beat both the quasilinear baselines and this null baseline before being promoted. This supports the next saturation-model lane while preventing premature absolute quasilinear transport claims.
   ...
   * - Quasilinear holdout-gap report
     ...
     - current artifact base: ``docs/_static/quasilinear_holdout_gap_report.png`` with CSV/JSON/PDF companions. It keeps ``absolute_flux_promoted=false`` and records both ``absolute_flux_promotion_requirements`` and ``screening_promotion_requirements`` blocks. The current absolute train/holdout error is about ``1.91`` against the ``0.35`` gate, i.e. about ``5.45`` times too large, with Cyclone Miller as the worst admitted holdout. The full-portfolio screening gate passes for ``spectral_envelope_ridge``, but the held-out-only screening gate remains below threshold (about ``0.71`` Spearman and ``0.73`` pairwise order accuracy). The CTH-like high-grid admission satisfies the external-VMEC-family and non-axisymmetric external-VMEC coverage requirements; the report still requires two additional independent passed holdouts and a substantially better saturation model before absolute-flux or held-out screening promotion can be reconsidered.
