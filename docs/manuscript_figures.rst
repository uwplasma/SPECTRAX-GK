...
.. list-table::
   ...
   * - Solver-objective geometry-gradient gate
     - ``tools/build_solver_objective_gradient_gate.py`` and ``tools/build_vmec_boozer_solver_frequency_gradient_gate.py``
     - Solver-ready production linear-RHS gradient gate closed; mode-21 VMEC/Boozer state-to-solver eigenfrequency gate closed; full quasilinear/nonlinear state-gradient promotion open
     - current artifact bases: ``docs/_static/solver_objective_gradient_gate.png`` and ``docs/_static/vmec_boozer_solver_frequency_gradient_gate.png`` with CSV/JSON/PDF companions. The first differentiates actual electrostatic linear-RHS eigenpair observables with respect to solver-ready geometry arrays using the implicit left/right eigenpair system and checks ``gamma``, ``omega``, ``<k_perp^2>``, linear heat/particle-flux weights, and a mixing-length heat-flux proxy against nearest-branch central finite differences. The second starts from a real ``vmec_jax`` state coefficient, maps through ``booz_xform_jax`` with ``mboz=nboz=21``, builds the SPECTRAX-GK linear RHS, and verifies the eigenfrequency gradient against finite differences. The quasilinear flux-weight state-gradient gate remains open because the current full-chain diagnostic is too heavy for default release tests.
   * - Inverse/UQ figure
