...
- Installed package: 289 Python files and about 101.3k LOC. The main blockers
  are `validation/benchmarks` (13 files), `objectives` (41 files),
...
| `src/spectraxgk/validation` | 14 files, all benchmark-related | close this package next by moving benchmark policy to a clearer benchmark-cases owner or root benchmark drivers |
...
1. **Shared helper ownership.** Move or delete the small helper modules that are
   ...
   `spectraxgk.benchmarks`. Remaining helpers are `defaults.py` and
   `harness.py`. Reusable physics or
...
- The repository inventory now leaves 16 installable validation files as
  `move-or-shrink`, with the broader test/tool/doc-artifact inventory still
  in the `keep-or-merge` and `keep-and-consolidate` queues. The 16
...
| --- | ---: | --- |
| `src/spectraxgk` | 289 Python files, about 101.3k LOC | installable package still contains benchmark validation policy plus many public/internal facades |
| `src/spectraxgk/validation` | 14 Python files, about 14.3k LOC | benchmark validation policy is still installed as runtime code |
...
| --- | ---: | ---: | --- |
| Installable source Python files | 289 | <= 100 | Move validation/campaign code out of `src`; consolidate domain modules. |
...
| `src/spectraxgk/validation` package | 14 | 0-5 | Remove installable validation campaigns; keep only tiny public metric helpers if necessary. |
...
- The active topology is `src/spectraxgk`: 289 Python files,
