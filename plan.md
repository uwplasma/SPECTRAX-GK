...
| Test consolidation | 97% | The enforced 95-file topology target is met, and the suite has 92,516 lines after current-API campaign contracts replaced removed-optimizer compatibility tests, status-dashboard metrics moved to exact mapping-subset assertions, installed-wheel/nonlinear-gate/Miller-cache regressions were added, and nonlinear-gradient, stellarator-transport, imported-VMEC, and autodiff tests stopped importing one owner under multiple aliases. Large merged objective, parallel, artifact, and reference-comparison owners now keep shared imports once at the file boundary. Corrupt nonlinear transport, quasilinear ensemble, persisted window, matched optimization, gradient-evidence artifacts, and consolidated diagnostics export contracts have explicit fail-closed or ownership coverage. The largest artifact-tool owner is 5,770 lines and the runtime runner is 4,222; status evidence and seven EIK geometry protocols use shared fixtures while retaining every physics/normalization distinction. Continue collapsing repeated tool/campaign contracts without reducing physics or coverage gates. |
| Source consolidation | 99.4% | The 223-file/zero-exception architecture gate passes after deleting redundant facades, merging nonlinear Laguerre transforms into their velocity-basis owner, reducing duplicated solver/cache/workflow/gradient/nonlinear-report policy, and removing private compatibility exports and obsolete optimizer adapters from solver, operator, VMEC transport, and nonlinear-parallel facades. The package has 87,261 lines, no inert merged-module strings, and no module at the 1,000-line ceiling; consolidated diagnostics owners now each expose one complete export contract instead of overwritten section-local lists. Continue reducing near-ceiling owners without flattening meaningful staging boundaries. |
| Structured solver ownership | 97% | Dtype-aware Arnoldi breakdown and true shifted-system residual retries close false convergence. Retained-Ritz, one-vector Jacobi--Davidson, ordered-Schur thick restart, and alternative-seed repairs were physically rejected. A two-sided interior correction or field-coupled low-moment preconditioner remains before broad KBM branch promotion. |
