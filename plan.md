...
| Source consolidation | 100% | The zero-exception architecture gate passes at 220 installable modules after deleting redundant facades, merging nonlinear Laguerre transforms into their velocity-basis owner, reducing duplicated solver/cache/workflow/gradient/nonlinear-report policy, and removing private compatibility exports and obsolete optimizer adapters. Collision equations remain in the 1,000-line ``operators/linear/collisions.py`` owner; generic tabulated-matrix interpolation now has a focused 140-line ``collision_tables.py`` owner. This keeps the new equal-species finite-wavelength runtime below the line ceiling without compatibility aliases or duplicated equations and remains below the 223-module topology target. Single-consumer QA, stellarator residual, growth-series, and VMEC boundary-chain policy splits remain consolidated. Reusable contracts, fit/window algorithms, tables, tensor mappings, collision equations, and solved-state sensitivity retain separate owners because they have independent consumers or numerical responsibilities. Future merges require fan-in evidence and must not trade domain clarity for file-count reduction. |
...
| Advanced collision operators | 98% | The shipped model has independent drift-kinetic and finite-b equation, dissipation, asymptotic, and AD gates. Published drift-kinetic original/improved-Sugama and Coulomb low-order matrices pass exact coefficient, null-space, symmetry, dissipation, invariant, and derivative gates. Ordered pairs support unequal mass/temperature species, conserve physical multispecies invariants in the drift-kinetic limit, and approach the original-Sugama collision null space in a time-domain relaxation gate. The improved correction passes its independent equal-species endpoint, matrix-wide equal-temperature dissipation, and heat-flow proximity-to-Coulomb gates. An 80-digit generator, checksummed package table, and device-side finite-b interpolation support shared, per-species, ordered target/source pairs, and an exact one-species diagonal path. Full-pair/diagonal identity, JIT/JVP interpolation, full-RHS, and fail-closed species gates pass. Coulomb, original-Sugama, and improved-Sugama conductivity hierarchies are converged through P=20, J=5 and K=5; the exact paper-wavelength slab-ITG hierarchy reaches :math:`(P,J)=(15,6)`. The physical finite-b zonal hierarchy now reaches P21/J8: both kx=0.1 criteria pass, while the kx=0.2 relative-L2 change is 5.60% and remains narrowly open. The complete drift-kinetic Figure-12 zonal gate is closed. Equal-species finite-wavelength original and improved-Sugama equations now pass their drift-limit, flow-restoration, nested quadrature, archive, JIT, and JVP gates. Promotion remains blocked by P24/J10 field-line table completion and traces plus the Figure-13/14 zonal-ordering and velocity-section gates. The first one-``(B,m)`` P24 block completed in 157.26 seconds, so the exact resumable campaign is now running rather than extending a monolithic timeout. |
...
- 2026-07-16: Closed the single-block P24/J10 feasibility gate on office.
  The exact ``B=0.12,m=4`` shard completed in 157.26 seconds: 61.45 seconds
  for the shared speed precompute and 95.81 seconds for matrix/polarization
  assembly, with checksum ``-3.1607491224410484e-05`` and 217 MB peak parent
...
- 2026-07-16: Completed the first exact P21/J8 four-wavelength Coulomb table
  ...
  checksummed, and uses the runtime Laguerre convention. Both matched 8,020-step
  physical zonal traces remain finite with imaginary fractions below
  ``8e-16``. From P18/J7 to P21/J8, ``kx=0.1`` changes by 2.37% relative L2 and
  1.58% maximum, passing both gates; ``kx=0.2`` changes by 5.60% relative L2
  and 3.09% maximum, so only the former narrowly fails. The tracked hierarchy
  is updated but remains open for P24/J10 without relaxing the 5% contract.
...
- 2026-07-16: The second hard-bounded P24/J10 endpoint attempt used 16 fork
  ...
  as negative performance evidence; no third identical run is planned. The
  runtime audit also showed that :math:`B=\sqrt{2b}` is not constant on the
  Miller field line: it spans ``0.12677--0.15496`` for ``kx=0.1`` and
  ``0.25353--0.30991`` for ``kx=0.2`` at ``Nz=32``. A fixed endpoint therefore
  cannot support the paper trace. A prospective fork decomposition of the
  gyroaveraged polarization coefficients was bitwise exact but delivered only
  ``1.04x`` at P12/J5 because each process rebuilt transform caches; it was
  rejected rather than retained as ineffective parallel complexity.

- 2026-07-16: Added the physically exact one-species diagonal finite-wavelength
  path. ``EqualSpeciesFiniteWavelengthCoulombOperator`` retains the complete
  Coulomb test, field, and four polarization terms while interpolating only
  the realizable :math:`B_a=B_b` coefficients. It is identical to the full
  pair equations on the diagonal, its JVP agrees with centered differences,
  and multispecies input fails closed. The shared-cache diagonal archive
  generator records a complete ordered B grid, prints every expensive phase,
  uses the runtime Laguerre convention, and passes replay/shape/finite-value
  gates. Generic table interpolation moved into a focused 140-line owner;
  the architecture gate passes at 220 modules with ``collisions.py`` at the
  1,000-line ceiling. The next scientific action is nested B-grid convergence
...
- 2026-07-16: Added a scoped POSIX-fork decomposition for the dominant exact
  ...
  route, not a general strong-scaling claim.

- 2026-07-16: Closed the paper-protocol finite-wavelength slab-ITG gate by the
  planned equivalent-growth route. Eight matrix workers completed the exact
  :math:`(15,6)`, :math:`B=1/\sqrt{2}` pair table in 261.39 seconds, with
  checksum ``-423.2127750524206`` and finite values throughout. From
  :math:`(12,5)` to :math:`(15,6)`, the maximum growth change is 1.99% over
  all collision frequencies and 0.037% over the unstable
  :math:`\nu\geq0.03` interval. The independent collisionless
  :math:`(15,6)\rightarrow(18,6)` endpoint changes by 0.59%. All three values
  pass the fixed 5% protocol, so a finite-collision P18 table is unnecessary
  for this scoped slab result. The publication artifact now reports
  ``CONVERGED``. Input-file promotion remains blocked on the independent
  collisional zonal-response gate.
...
- 2026-07-16: Rebuilt the exact paper-wavelength P7, P9, and P12 hierarchy with
  ...
  557.87 seconds; the P12 gain is 4.00x without a precision change. An initial
  P15/J6 serial probe reached matrix completion in 350.84 seconds but was
  stopped before polarization at the local five-minute boundary. The
  subsequent scoped row decomposition completed that table and closed the
  equivalent-growth gate as recorded above.
