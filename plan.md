...
1. **Shared helper ownership.** Move or delete the small helper modules that are
   ...
   in `diagnostics/growth_rates.py`. Remaining helpers are `batching.py`,
   `solver_policy.py`, `initialization.py`, `reference.py`, `species.py`,
   `scan.py`, `defaults.py`, `harness.py`, and `harness_scan.py`. Reusable physics or
...
- Current topology counts:
  - `src/spectraxgk`: 306 Python files after extracting nonlinear-gradient, nonlinear-transport, stellarator validation subpackages, benchmark case presets, benchmark eigenfunction diagnostics, benchmark time-series/window diagnostics, benchmark zonal-response metrics, benchmark trace/window metrics, and benchmark fit-signal helpers.
...
- `src/spectraxgk/validation`: 31 installable benchmark-validation files.
...
| --- | ---: | --- |
| `src/spectraxgk` | 306 Python files, about 101.7k LOC | installable package still contains benchmark validation policy plus many public/internal facades |
| `src/spectraxgk/validation` | 31 Python files, about 15.8k LOC | benchmark validation policy is still installed as runtime code |
...
| --- | ---: | --- | --- |
| `validation.benchmarks` | 30 files, about 15.8k LOC | benchmark case runners, fit policy, branch ladders, and the current implementation behind `spectraxgk.benchmarks` | small public `spectraxgk.benchmarks` facade plus root `benchmarks/` drivers and `tests/validation/benchmarks` policy tests |
...
| `src/spectraxgk/validation` package | 31 | 0-5 | Remove installable validation campaigns; keep only tiny public metric helpers if necessary. |
