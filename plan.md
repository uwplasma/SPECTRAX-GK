...
| Test consolidation | 97.4% | The enforced 95-file topology target is met, and the suite has 92,334 lines after current-API campaign contracts replaced removed-optimizer compatibility tests, status-dashboard metrics moved to exact mapping-subset assertions, installed-wheel/nonlinear-gate/Miller-cache regressions were added, and nonlinear-gradient, stellarator-transport, imported-VMEC, and autodiff tests stopped importing one owner under multiple aliases. Large merged objective, parallel, artifact, and reference-comparison owners now keep shared imports once at the file boundary; VMEC/Boozer aggregate artifact gates share one call-capture contract. Corrupt nonlinear transport, quasilinear ensemble, persisted window, matched optimization, gradient-evidence artifacts, and consolidated diagnostics export contracts have explicit fail-closed or ownership coverage. The largest artifact-tool owner is 5,723 lines and the runtime runner is 4,088; status evidence, seven EIK geometry protocols, and runtime integrator call capture use shared fixtures or keyword stubs while retaining every physics/normalization distinction. Continue collapsing repeated tool/campaign contracts without reducing physics or coverage gates. |
