...
### 2026-06-04 Full Max-Mode-5 QA Projected Nonlinear Audits

- Office campaign root: `/home/rjorge/tmp/spectrax_vmec_qa_full_20260603_172124/SPECTRAX-GK`.
- Completed the matched `n64`, `mboz=nboz=21`, `t=[350,700]` nonlinear audits for the full max-mode-5 QA baseline and projected/admission transport-weight `5e-4` and `1e-3` candidates.
- Baseline and both projected candidate seed/timestep ensembles pass their stationarity/replicate gates.
- Matched comparison results:
  - baseline late-window mean `Q_i = 9.695`;
  - projected `5e-4`: `Q_i = 9.436`, relative reduction `2.68%`, uncertainty separation `z = 1.32`;
  - projected `1e-3`: `Q_i = 9.370`, relative reduction `3.35%`, uncertainty separation `z = 1.56`.
- Added publication-facing artifacts: `docs/_static/vmec_jax_qa_full_sweep_panel.{png,json,csv}`, `docs/_static/vmec_jax_qa_projected_weight_0p0005_matched_comparison.{png,json}`, and `docs/_static/vmec_jax_qa_projected_weight_0p001_matched_comparison.{png,json}`.
- Scope: these are positive single-surface, single-field-line, single-`ky` nonlinear transport audits. They do not yet promote broad stellarator turbulent-flux optimization; that requires the planned multi-surface, multi-alpha, multi-`ky` promotion gate.
- Remaining near-term step: let the office queue continue any direct growth/QL/nonlinear-window candidate audits requested for optimizer-algorithm comparison, but do not promote failed solved-WOUT branches as optimized stellarators.
