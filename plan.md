...
- 2026-07-16: Advanced the finite-wavelength Coulomb generator to the first
  required-wavelength intermediate hierarchy point without weakening its
  ...
  :math:`k_\perp\rho=0.5` build from 464.05 to 183.59 seconds on the same
  office CPU. All six arrays retain checksum -152.93360627939981 and finite
  values. The common :math:`(7,3)` block is not converged: relative matrix and
  nonzero-polarization changes remain greater than one. The tracked hierarchy
  artifact therefore admits generator feasibility only and keeps both
  :math:`(18,6)` resolution and transport acceptance false. The next bounded
  action is one :math:`(12,5)` build, followed by the literature geometry and
  branch gate; brute-force promotion is forbidden.
