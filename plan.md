...
2. **Finite-truncation convergence (drift-kinetic response closed; finite-b hierarchy open).** Track
   ...
   :math:`b` hierarchy or absolute transport normalization.
...
4. **Strongly collisional transport (normalized Coulomb response closed;
   absolute coefficients open).** Reproduce Spitzer--Härm parallel
...
- 2026-07-16: Closed the normalized drift-kinetic Coulomb response hierarchy.
  ...
  artifacts are tracked and regenerated from equations. Absolute Spitzer--Härm
  conductivity remains open pending matched dimensional normalization,
  saturation/field-amplitude checks, and arbitrary-order original/improved-
  Sugama hierarchies. A separate post-release nonlinear Landau plan follows
