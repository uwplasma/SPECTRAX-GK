...
| Test consolidation | 97.2% | The enforced 95-file topology target is met, and the suite has 92,468 lines after current-API campaign contracts replaced removed-optimizer compatibility tests, status-dashboard metrics moved to exact mapping-subset assertions, installed-wheel/nonlinear-gate/Miller-cache regressions were added, and nonlinear-gradient, stellarator-transport, imported-VMEC, and autodiff tests stopped importing one owner under multiple aliases. Large merged objective, parallel, artifact, and reference-comparison owners now keep shared imports once at the file boundary; VMEC/Boozer aggregate artifact gates share one call-capture contract. Corrupt nonlinear transport, quasilinear ensemble, persisted window, matched optimization, gradient-evidence artifacts, and consolidated diagnostics export contracts have explicit fail-closed or ownership coverage. The largest artifact-tool owner is 5,723 lines and the runtime runner is 4,222; status evidence and seven EIK geometry protocols use shared fixtures while retaining every physics/normalization distinction. Continue collapsing repeated tool/campaign contracts without reducing physics or coverage gates. |
| Source consolidation | 99.5% | The 223-file/zero-exception architecture gate passes after deleting redundant facades, merging nonlinear Laguerre transforms into their velocity-basis owner, reducing duplicated solver/cache/workflow/gradient/nonlinear-report policy, and removing private compatibility exports and obsolete optimizer adapters from solver, operator, VMEC transport, and nonlinear-parallel facades. The package has 87,246 lines, no inert merged-module strings, and no module at the 1,000-line ceiling; consolidated diagnostics owners expose complete export contracts, and term assembly shares the collision owner's dtype-aware trace-time zero policy. Continue reducing near-ceiling owners only where a real policy owner exists, without flattening meaningful staging boundaries. |
| Structured solver ownership | 97% | Dtype-aware Arnoldi breakdown and true shifted-system residual retries close false convergence. Retained-Ritz, one- and two-sided Jacobi--Davidson, ordered-Schur thick restart, alternative seeds, and a nested matrix-free field block were physically rejected. Broad KBM branch promotion now requires an analytically reduced field/moment Schur preconditioner. |
