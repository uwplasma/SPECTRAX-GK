...
| Test consolidation | 97.5% | The enforced 95-file topology target is met, and the suite has 92,256 lines after current-API campaign contracts replaced removed-optimizer compatibility tests, status-dashboard metrics moved to exact mapping-subset assertions, installed-wheel/nonlinear-gate/Miller-cache regressions were added, and nonlinear-gradient, stellarator-transport, imported-VMEC, and autodiff tests stopped importing one owner under multiple aliases. Large merged objective, parallel, artifact, and reference-comparison owners now keep shared imports once at the file boundary; VMEC/Boozer aggregate artifact gates share one call-capture contract. Corrupt nonlinear transport, quasilinear ensemble, persisted window, matched optimization, gradient-evidence artifacts, and consolidated diagnostics export contracts have explicit fail-closed or ownership coverage. The largest artifact-tool owner is 5,723 lines and the runtime runner is 4,088; status evidence, seven EIK geometry protocols, runtime integrator call capture, the physical ITG eigen-objective, and executable result payloads use shared fixtures while retaining every physics/normalization and AD-method distinction. Continue collapsing repeated tool/campaign contracts without reducing physics or coverage gates. |
| Source consolidation | 99.8% | The zero-exception architecture gate passes at 220 installable modules after deleting redundant facades, merging nonlinear Laguerre transforms into their velocity-basis owner, reducing duplicated solver/cache/workflow/gradient/nonlinear-report policy, and removing private compatibility exports and obsolete optimizer adapters from solver, operator, VMEC transport, and nonlinear-parallel facades. The package has 87,119 lines, no inert merged-module strings, and no module at the 1,000-line ceiling; QA low-turbulence contracts, equations, residuals, and sensitivity gates have one 653-line model owner, while the reduced stellarator ITG equations and their residual/conditioning gates have one 637-line owner. Consolidated diagnostics expose complete export contracts, persisted evidence uses one strict Boolean/finite/count decoder, and term assembly shares the collision owner's dtype-aware trace-time zero policy. Continue reducing only where a real single-consumer policy owner exists, without flattening independently reusable contracts, tables, or orchestration stages. |
