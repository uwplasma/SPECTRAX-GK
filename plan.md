...
- Current topology counts:
  - `src/spectraxgk`: 341 Python files after consolidating nonlinear transport-window and quasilinear-calibration diagnostics.
...
- The repository inventory classifies 73 installable validation files as
  `move-or-shrink`, 503 files as `keep-or-merge`, 268 files as
  `keep-and-consolidate`, 82 files as `keep-or-scope`, and 26 files as
  `keep-or-review`. The 73 `move-or-shrink` files are the validation package;
...
- The remaining code-size problem is family sprawl:
  ...
  - `src/spectraxgk/validation`: 73 installable validation/campaign files.
...
Current measured topology after the nonlinear-window and quasilinear-calibration consolidation tranches:

| Area | Files / lines | Main issue |
| --- | ---: | --- |
| `src/spectraxgk` | 341 Python files, about 106.0k LOC | installable package still contains validation campaigns and many public/internal facades |
| `src/spectraxgk/validation` | 73 Python files, about 28.2k LOC | campaign, benchmark, nonlinear-gradient, remaining quasilinear, and stellarator validation policy is installed as runtime code |
...
| --- | ---: | ---: | --- |
| Installable source Python files | 341 | <= 100 | Move validation/campaign code out of `src`; consolidate domain modules. |
...
| `src/spectraxgk/validation` package | 73 | 0-5 | Remove installable validation campaigns; keep only tiny public metric helpers if necessary. |
...
| --- | --- | --- | --- |
| Validation in `src` | 73 installable files, many are campaign/report builders | Move benchmark/campaign code to `benchmarks/`, `tools/campaigns`, or `tests/validation`; keep only reusable metrics or public facades | Largest source-file reduction and cleaner runtime imports |
...
- The active topology is `src/spectraxgk`: 341 Python files,
...
- The largest structural source offender is still
  `src/spectraxgk/validation`: 73 installable Python files and about 28.2k
  Python lines.
...
4. **Move validation campaigns out of the installable package.** Keep reusable
   ...
   `tests/validation`. Target: `src/spectraxgk/validation` from 73 files to at
   most 5.
...
Current problem: `src/spectraxgk` has 341 Python files, and 73 are validation
