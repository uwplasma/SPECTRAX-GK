...
- 2026-07-15: Added the first coupled finite-wavelength moment-resolution gate.
  ...
  Against a converged :math:`(9,4)` reference, :math:`(6,3)` passes the fixed
  :math:`10^{-3}` gate at :math:`4.695\times10^{-4}`, and :math:`(8,4)` reaches
  :math:`8.685\times10^{-7}`. The JSON, publication panel, and fast tracked
  test now fail closed on Bessel, spherical, and radial truncation separately;
  complete regeneration finishes in 207 seconds under the five-minute policy.
