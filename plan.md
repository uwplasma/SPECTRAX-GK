...
| Advanced collision operators | 95% | The shipped model has independent drift-kinetic and finite-b equation, dissipation, asymptotic, and AD gates. Published drift-kinetic original/improved-Sugama and Coulomb low-order matrices pass exact coefficient, null-space, symmetry, dissipation, invariant, and derivative gates. Ordered pairs support unequal mass/temperature species, conserve physical multispecies invariants in the drift-kinetic limit, and approach the original-Sugama collision null space in a time-domain relaxation gate. The improved correction passes its independent equal-species endpoint, matrix-wide equal-temperature dissipation, and heat-flow proximity-to-Coulomb gates. An 80-digit generator, checksummed package table, and device-side finite-b interpolation support shared, per-species, ordered target/source pairs, and an exact one-species diagonal path. Full-pair/diagonal identity, JIT/JVP interpolation, full-RHS, and fail-closed species gates pass. Coulomb, original-Sugama, and improved-Sugama conductivity hierarchies are converged through P=20, J=5 and K=5; the exact paper-wavelength slab-ITG hierarchy reaches :math:`(P,J)=(15,6)`. The complete drift-kinetic Figure-12 zonal gate is closed. Promotion remains blocked by three explicit items: tractable P24/J10 field-line Coulomb table generation plus nested B-grid convergence, finite-wavelength original/improved-Sugama equations, and the Figure-13/14 trace and velocity-section gates. Two hard-bounded P24/J10 endpoint attempts exceeded 600 seconds and produced no retained artifact, so the status is reduced rather than treating algebra plumbing as completed physics evidence. |
