...
- Current source-simplification head:
  `5ba5fb0d Simplify late-time linear metrics`.
- Worktree at audit start: local `main` is one commit ahead of `origin/main`
  with the late-time linear metrics simplification tranche; this plan update is
  the only pending repo-level change.
- Latest CI state at audit: the newest pushed `main` CI run is queued on
  `a6df7ee3`; verify the newest non-superseded run before release tagging, but
...
- Function-length audit: 0 source functions at or above 90 lines, 24 functions
  in the 80-89 line band, and 106 functions at or above 70 lines. Long classes
...
- Recent refactors simplified runtime artifact display, gradient-validation
  ...
  late-time linear metrics signal/tail-stat assembly.
...
| P1 | Source simplification and naming | 99.2% | No new root modules, zero functions >=90 lines, 24 functions in the 80-89 band, and next tranches reduce file/navigation sprawl instead of adding thin seams. |
| P1 | Refactor/testability | 98.8% | Remaining 80-89 line functions reduced only when they expose real physics/numerics policy boundaries, remove duplication, or consolidate single-use wrappers. |
...
4. Take one final small source-simplification tranche only if it removes a real
   ...
   `workflows/nonlinear.py::run_full_nonlinear_runtime`,
   `solvers/time/diffrax_linear.py::integrate_linear_diffrax`, or a thin
