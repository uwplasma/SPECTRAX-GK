"""Readers for legacy grouped cETG NetCDF outputs."""
...
class LegacyCetgOutput:
    """Minimal legacy cETG diagnostic contract."""
...
@dataclass(frozen=True)
class LegacyCetgRestart:
    """Legacy cETG restart state in the positive-ky in-memory layout."""
...
def expand_legacy_positive_ky_state(
    state_positive_ky: np.ndarray,
    *,
    ny_full: int,
) -> np.ndarray:
    ...
    if nx > 1:
        kx_neg = np.concatenate(
            [np.array([0], dtype=np.int32), np.arange(nx - 1, 0, -1, dtype=np.int32)]
        )
        neg = neg[:, :, kx_neg, :]
...
def load_legacy_cetg_restart(
    path: str | Path,
    *,
    nx_full: int,
    ny_full: int,
) -> LegacyCetgRestart:
    """Load a legacy cETG restart file into the positive-ky in-memory layout."""

    try:
        from netCDF4 import Dataset
    except ImportError as exc:  # pragma: no cover
        raise ImportError(
            "netCDF4 is required to load legacy cETG restart files"
        ) from exc
    ...
    try:
        ...
        if raw.ndim != 7:
            raise ValueError(f"Legacy cETG restart G has unsupported rank {raw.ndim}")
        nspec, nm, nl, nz, nakx, naky, ri = raw.shape
        if nspec != 1 or nm != 1 or nl != 2 or ri != 2:
            raise ValueError(
                "Legacy cETG restart must have shape (1, 1, 2, Nz, Nkx, Nky, 2); "
                f"got {raw.shape}"
            )
        ...
        state = np.zeros(
            (1, nl, 1, nyc_full, int(nx_full), int(nz)), dtype=np.complex64
        )
...
def load_legacy_cetg_output(path: str | Path) -> LegacyCetgOutput:
    """Load the grouped legacy cETG NetCDF format."""

    try:
        from netCDF4 import Dataset
    except ImportError as exc:  # pragma: no cover
        raise ImportError("netCDF4 is required to load legacy cETG outputs") from exc
