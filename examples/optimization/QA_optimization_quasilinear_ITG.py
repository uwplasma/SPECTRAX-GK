...
OUTPUT_DIR = Path("output_QA_optimization_quasilinear_ITG")
...
def transport_objective(state, runtime):
    """Mixing-length quasilinear ITG heat-flux proxy on one flux tube."""

    return turb.quasilinear_flux_proxy(
        state,
        runtime,
...
print(f"quasilinear ITG proxy: {flux_seed:.6e} -> {flux_final:.6e}")

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
inp.to_indata(OUTPUT_DIR / "input.QA_quasilinear_ITG_optimized")
wout_path = vj.write_wout(OUTPUT_DIR / "wout_QA_quasilinear_ITG_optimized.nc", eq.wout)
for name, path in vj.plot_wout(wout_path, OUTPUT_DIR).items():
    print(f"wrote {name}: {path}")
print("Treat this as screening evidence; validate with matched long nonlinear windows.")
