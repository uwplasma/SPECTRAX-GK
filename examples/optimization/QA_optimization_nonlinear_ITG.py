...
OUTPUT_DIR = Path("output_QA_optimization_nonlinear_ITG")
...
def transport_objective(state, runtime):
    """Reduced nonlinear-window ITG heat-flux proxy on one flux tube."""

    return turb.nonlinear_heat_flux_proxy(
        state,
        runtime,
        csat=SATURATION_COEFFICIENT,
...
def report(tag, equilibrium):
    ...
    flux = float(transport_objective(equilibrium.state, equilibrium.runtime))
    print(
        ...
        f"Q_NL_proxy={flux:.6e}"
    )
    return flux


flux_seed = report("seed", eq)
objective_terms = [
    (qs, 0.0, 1.0),
...
flux_final = report("final", eq)
print(f"reduced nonlinear-window proxy: {flux_seed:.6e} -> {flux_final:.6e}")

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
inp.to_indata(OUTPUT_DIR / "input.QA_nonlinear_ITG_optimized")
wout_path = vj.write_wout(OUTPUT_DIR / "wout_QA_nonlinear_ITG_optimized.nc", eq.wout)
for name, path in vj.plot_wout(wout_path, OUTPUT_DIR).items():
    print(f"wrote {name}: {path}")
print(
    "Run QA_nonlinear_ITG_matched_audit.py for long post-transient promotion evidence."
)
