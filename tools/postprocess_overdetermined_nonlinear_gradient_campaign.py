...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    if payload.get("kind") != "overdetermined_nonlinear_turbulence_gradient_campaign_manifest":
        raise ValueError(
...
def build_postprocess_commands(
    ...
    *,
    ...
    for control in manifest["controls"]:
        ...
        if not nested_raw:
            raise ValueError(f"control {slug!r} is missing expected_nonlinear_campaign_manifest")
        ...
        commands.append(
            PlannedCommand(
                ...
                " ".join(
                    [
                        "python3",
                        "tools/run_nonlinear_gradient_manifest_postprocess.py",
    ...
    commands.append(PlannedCommand("candidate-ranking", "promotion", str(ranking_command)))
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary_json = args.summary_json or (
        (status_json or _default_status_json(manifest_path)).with_name("overdetermined_postprocess_summary.json")
    )
    _write_summary(
        path=summary_json,
        manifest_path=manifest_path,
