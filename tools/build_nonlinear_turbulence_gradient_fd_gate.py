...
DEFAULT_OUT_PREFIX = ROOT / "docs" / "_static" / "nonlinear_turbulence_gradient_central_fd_gate"
...
def write_artifacts(report: dict[str, Any], out_prefix: Path) -> dict[str, str]:
    ...
    means = np.asarray([_float_or_nan(source_ensembles.get(label, {}).get("ensemble_mean")) for label in labels])
    sems = np.asarray([_float_or_nan(source_ensembles.get(label, {}).get("combined_sem")) for label in labels])
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"passed": report["passed"], "blockers": report["blockers"], "paths": paths}, indent=2))
