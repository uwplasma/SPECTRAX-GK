...
DEFAULT_OUT = ROOT / "docs" / "_static" / "vmec_boozer_aggregate_line_search_comparison.png"
...
def _summarize_report(objective: str, report: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "absolute_reduction": initial - final if math.isfinite(initial) and math.isfinite(final) else math.nan,
...
def build_vmec_boozer_aggregate_line_search_comparison_report(
    ...
    **kwargs: Any,
    ...
    rows = [_summarize_report(objective, reports[objective]) for objective in objective_tuple]
    same_samples = _samples_match(reports)
    directions = [str(row["initial_update_direction"]) for row in rows]
    same_direction = bool(directions and all(direction == directions[0] for direction in directions[1:]))
    all_line_searches_passed = all(bool(row["passed"]) for row in rows)
    final_deltas = [_finite_float(row["final_delta"]) for row in rows]
    delta_spread = float(max(final_deltas) - min(final_deltas)) if all(math.isfinite(item) for item in final_deltas) else None
    ...
    reduction_spread = float(max(finite_reductions) - min(finite_reductions)) if finite_reductions else None
    ...
    return {
        ...
        "initial_update_directions": {str(row["objective"]): str(row["initial_update_direction"]) for row in rows},
...
def write_vmec_boozer_aggregate_line_search_comparison_artifacts(
    ...
    *,
    ...
    labels = [str(row.get("objective", index)) for index, row in enumerate(row_list) if isinstance(row, dict)]
    initial = np.asarray([_finite_float(row.get("initial_objective")) for row in row_list if isinstance(row, dict)], dtype=float)
    final = np.asarray([_finite_float(row.get("final_objective")) for row in row_list if isinstance(row, dict)], dtype=float)
    rel = np.asarray([_finite_float(row.get("relative_reduction")) for row in row_list if isinstance(row, dict)], dtype=float)
    derivative = np.asarray([_finite_float(row.get("initial_central_derivative")) for row in row_list if isinstance(row, dict)], dtype=float)
    final_delta = np.asarray([_finite_float(row.get("final_delta")) for row in row_list if isinstance(row, dict)], dtype=float)
    finite_initial = np.where(np.isfinite(initial) & (np.abs(initial) > 0.0), initial, np.nan)
    ...
    colors = ["#ca6702" if sign > 0.0 else "#0a9396" if sign < 0.0 else "#6b7280" for sign in signs]
    if labels:
        ...
        for xi, deriv in zip(x, derivative, strict=True):
            if math.isfinite(float(deriv)):
                ax_dir.text(xi, 0.0, f"dJ/dx={deriv:.2g}", ha="center", va="bottom", fontsize=7.5, rotation=90)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json_only:
        ...
        return 0
    paths = write_vmec_boozer_aggregate_line_search_comparison_artifacts(payload, out=args.out)
