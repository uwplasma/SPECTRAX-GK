...
def _parse_coefficient_spec(raw: str) -> CoefficientSpec:
    match = re.fullmatch(r"\s*(RBC|RBS|ZBC|ZBS)\(\s*([+-]?\d+)\s*,\s*([+-]?\d+)\s*\)\s*", raw)
    if match is None:
        raise ValueError("coefficient must have format RBC(m,n), RBS(m,n), ZBC(m,n), or ZBS(m,n)")
...
def _coefficient_value(text: str, spec: CoefficientSpec) -> float:
    ...
    if not rows:
        raise ValueError(f"coefficient {spec.label} not found")
    if len(rows) > 1:
        raise ValueError(f"coefficient {spec.label} appears {len(rows)} times; refusing ambiguous read")
...
def write_perturbation_inputs(
    *,
    ...
    for state, value in states.items():
        ...
        input_file.write_text(_patch_coefficient(text, coefficient, value), encoding="utf-8")
