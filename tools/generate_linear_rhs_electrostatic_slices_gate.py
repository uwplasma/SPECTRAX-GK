#!/usr/bin/env python3
"""Generate a composed electrostatic linear-slices RHS identity gate."""
...
DEFAULT_PREFIX = REPO_ROOT / "docs" / "_static" / "linear_rhs_electrostatic_slices_gate"
...
def build_problem(
    *,
    ...
    """Build a periodic electrostatic linear-RHS problem with all gated slices."""
        ...
        rows.append(
            {
                ...
                "abs_error": float(np.asarray(jnp.max(jnp.abs(sharded[:, m_idx, ...] - serial[:, m_idx, ...])))),
            }
        )
