...
def _transport_sample(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError("transport_sample missing required key(s): " + ", ".join(missing))
    return dict(sample)
...
def build_vmec_boozer_production_holdout_artifact(
    *,
    ...
    case_name = str(case or manifest.get("case") or ensemble.get("case") or manifest_path.parent.name)
    holdout_sample = {
        "surface_index": None,
    ...
    return {
        ...
        "promotion_gate": {
            "passed": passed,
            "blockers": [] if passed else ["replicated_nonlinear_window_ensemble_failed"],
        },
