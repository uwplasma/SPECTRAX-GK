...
DEFAULT_OUT_PREFIX = ROOT / "docs" / "_static" / "nonlinear_gradient_state_to_input_mapping_response"
STATE_RE = re.compile(r"^(?P<family>Rcos|Rsin|Zcos|Zsin)_(?:(?:mid_surface)|(?:r(?P<radial>[0-9]+)))_m(?P<mode>[0-9]+)$")
...
def _extract_state_vector_from_wout(
    ...
    for parameter in state_parameters:
        ...
        if not (0 <= radial_index < array.shape[0]) or not (0 <= mode_index < array.shape[1]):
            raise ValueError(f"{parameter} index is outside state array shape {array.shape}")
    ...
    *,
    ...
    for col, direction in enumerate(input_directions):
        ...
        if not math.isfinite(delta) or delta <= 0.0:
            raise ValueError(f"{coefficient} delta_parameter must be finite and positive")
        ...
        try:
            ...
            raise ValueError(f"{coefficient} is missing baseline/plus_delta/minus_delta samples") from exc
        for row, state_parameter in enumerate(state_parameters):
            jacobian[row, col] = (float(plus[state_parameter]) - float(minus[state_parameter])) / (2.0 * delta)
        direction_rows.append(
            {
                "coefficient": coefficient,
                "coefficient_slug": direction.get("coefficient_slug"),
                "delta_parameter": float(delta),
                "baseline_state": {key: float(baseline[key]) for key in state_parameters},
                "plus_state": {key: float(plus[key]) for key in state_parameters},
                "minus_state": {key: float(minus[key]) for key in state_parameters},
                "central_response": {
                    key: float(jacobian[row, col]) for row, key in enumerate(state_parameters)
                },
            }
        )
    ...
    if singular_values.size == 0 or float(np.min(singular_values)) <= float(response_floor):
    ...
    matrix_condition_ok = math.isfinite(condition_number) and condition_number <= float(max_condition_number)
    ...
    for row, state_parameter in enumerate(state_parameters):
        ...
        residual = float(np.linalg.norm(predicted - target) / max(np.linalg.norm(target), response_floor))
        row_has_response = bool(np.max(np.abs(jacobian[row, :])) > float(response_floor))
        residual_ok = residual <= float(max_relative_residual)
        passed = bool(matrix_has_response and row_has_response and matrix_condition_ok and residual_ok)
        blockers: list[str] = []
        ...
        controls.append(
            {
                ...
                "passed": passed,
                "input_control_argument": None if not passed else " ".join(f"{t['coefficient']}:{t['weight']:.12g}" for t in terms),
                ...
                "condition_number_label": "infinite" if not math.isfinite(condition_number) else f"{condition_number:.6g}",
                "relative_residual": float(residual),
                "dominant_response_sign": float(np.sign(jacobian[row, int(np.argmax(np.abs(jacobian[row, :])))]) if row_has_response else 0.0),
                "blockers": blockers,
            }
        )
...
def _samples_from_campaign(
    ...
    controls = [str(row["state_parameter"]) for row in campaign["admitted_state_controls"]]
...
def _write_csv(path: Path, report: Mapping[str, Any]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as stream:
        ...
        for label, values in zip(report["jacobian"]["row_order"], report["jacobian"]["matrix"]):
...
def _plot(path: Path, report: Mapping[str, Any]) -> None:
    ...
    residuals = np.asarray([float(row["relative_residual"]) for row in report["controls"]])
    set_plot_style()
    ...
    axes[1].axhline(float(report["acceptance"]["max_relative_residual"]), color="0.25", ls="--", lw=1.1)
    axes[1].set_xticks(x, rows, rotation=24, ha="right")
    axes[1].set_ylim(0.0, max(1.05, float(np.max(residuals)) * 1.1 if residuals.size else 1.0))
...
def main(argv: list[str] | None = None) -> int:
    ...
    json_path.write_text(json.dumps(clean, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
