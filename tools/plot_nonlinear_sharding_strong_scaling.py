...
DEFAULT_PREFIX = REPO_ROOT / "docs" / "_static" / "nonlinear_sharding_strong_scaling_large"
...
def write_artifacts(summary: dict[str, Any], out_prefix: Path) -> dict[str, str]:
    ...
    for backend in sorted({str(row["backend"]) for row in rows}):
        ...
        y = np.asarray([float(row["strong_speedup_vs_1_device"]) for row in subset], dtype=float)
        ...
        axes[1].semilogy(x, np.maximum(t, 1.0e-16), marker=marker, lw=2.2, color=color, label=label)
