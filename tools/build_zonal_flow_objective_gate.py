...
def _recurrence_value(
    *,
    ...
    if source == "tail_std":
        return None if comparison_row is None else _optional_float(comparison_row.get("tail_std"))
...
def _plot_payload(path: Path, payload: dict[str, object]) -> None:
    ...
    metrics = {
        "Residual response\n(higher is better)": [float(table[index]["residual_level"]) for index in order],
        "Damping penalty\n(lower is better)": [float(table[index]["damping_rate"]) for index in order],
        "Recurrence/tail penalty\n(lower is better)": [float(table[index]["recurrence_amplitude"]) for index in order],
        "Weighted sample objective\n(lower is better)": [float(table[index]["sample_objective"]) for index in order],
    }
    colors = ["#1f77b4", "#d62728", "#ff7f0e", "#2ca02c"]

    fig, axes = plt.subplots(2, 2, figsize=(9.2, 6.6), constrained_layout=True)
    for ax, (title, values), color in zip(axes.ravel(), metrics.items(), colors, strict=True):
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = zonal_flow_objective_artifact_from_records(
        ...
        source_paths=[_repo_relative(args.summary_csv), _repo_relative(args.comparison_csv)],
    )
    ...
    payload["validation_status"] = "closed" if payload["promotion_ready"] else "diagnostic"
