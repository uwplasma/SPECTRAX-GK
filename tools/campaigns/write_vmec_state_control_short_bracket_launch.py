...
from tools.campaigns.write_nonlinear_turbulence_gradient_campaign import (
    PYTHON_CMD,
    _repo_relative,
)  # noqa: E402
...
DEFAULT_OUT_PREFIX = (
    ROOT / "docs" / "_static" / "nonlinear_gradient_state_control_short_bracket_launch"
)
...
def _mapped_controls(
    runbook: Mapping[str, Any], state_parameter: str | None
    ...
    for row in rows:
        ...
        if (
            ...
            and row.get("state_parameter") != state_parameter
        ):
            continue
        out.append(row)
    if not out:
                ...
                writer.writerow(
                    {
                        ...
                        "inserted_missing_coefficient": control[
                            "inserted_missing_coefficient"
                        ],
...
def _plot(path: Path, report: Mapping[str, Any]) -> None:
    ...
    coeffs = sorted(
        {control["coefficient"] for row in launches for control in row["controls"]}
    )
    ...
    for i, row in enumerate(launches):
        lookup = {
            control["coefficient"]: float(control["coefficient_delta"])
            for control in row["controls"]
        }
    ...
    condition = np.asarray(
        [float(row["mapping_condition_number"]) for row in launches], dtype=float
    )
    residual = np.asarray(
        [float(row["mapping_relative_residual"]) for row in launches], dtype=float
    )
    ...
    *,
    ...
    for control in controls:
        ...
        launches.append(
            {
                ...
                "campaign_command_after_vmec_runs": clean[
        ...
        vmec_run_commands.extend(
            str(command) for command in clean["vmec_run_commands"].values()
        )
