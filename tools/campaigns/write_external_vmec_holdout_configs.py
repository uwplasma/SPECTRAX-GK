...
"""
    return f"""# Generated by tools/campaigns/write_external_vmec_holdout_configs.py.
# Case: {case}
...
def _write_command_script(path: Path, commands: list[str], *, title: str) -> None:
    """Write a fail-fast shell script for a generated launch command list."""

    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        f"# {title}",
        "# Generated by tools/campaigns/write_external_vmec_holdout_configs.py.",
        "# DEVICE selects the CUDA device used by each spectraxgk launch command.",
...
def write_manifest(out_dir: Path, written: list[WrittenConfig]) -> Path:
    ...
    manifest: dict[str, Any] = {
        ...
        "staged_ladder_skip_existing_script": "run_staged_ladder_skip_existing.sh",
        "direct_full_horizon_skip_existing_script": "run_direct_full_horizon_skip_existing.sh",
        "restart_ladder_note": (
    ...
    _write_command_script(
        ...
        staged_ladder_skip_existing_commands,
        title="external-VMEC staged restart ladder",
    )
    _write_command_script(
        out_dir / str(manifest["direct_full_horizon_skip_existing_script"]),
        direct_full_horizon_skip_existing_launch_commands,
