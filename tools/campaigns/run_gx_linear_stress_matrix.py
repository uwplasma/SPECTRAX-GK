...
def _case_defs(gx_repo: Path) -> dict[str, dict[str, Path]]:
    ...
    return {
        ...
        "cyclone_ke": {
            "gx_nc": bench
            / "ITG_cyclone"
            / "itg_miller_kinetic_electrons_correct.out.nc",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a comparison-code checkout (defaults to $REFERENCE_GK_REPO or common locations).",
    )
    p.add_argument(
        "--outdir",
        type=Path,
        default=Path("tools_out") / "stress_matrix_linear",
...
def _run_case(
    *, name: str, gx_nc: Path, gx_in: Path, out_csv: Path, Nl: int, Nm: int
) -> pd.DataFrame:
    if not gx_nc.exists():
...
def main() -> None:
    ...
    if gx_repo is None:
        raise SystemExit(
            "Could not find a comparison-code repo. Pass --gx-repo or set REFERENCE_GK_REPO."
        )
