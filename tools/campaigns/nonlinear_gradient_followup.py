...
from spectraxgk.diagnostics.nonlinear_gradient_statistics import (
    ...
    _mean_and_sem,
    _sample_covariance,
    _select_control_variate_candidate,
...
def _nested_metric(
    payload: Mapping[str, Any], source_name: str, *names: str
) -> float | None:
...
def _coefficient_label_from_parameter(parameter: Any) -> str | None:
    if not isinstance(parameter, str):
        return None
    match = re.fullmatch(
        r"\s*(rbc|rbs|zbc|zbs)_([+-]?\d+)_([+-]?\d+)\s*", parameter, re.IGNORECASE
    )
    if match is None:
        return None
...
def _candidate_bracket_estimates(
    ...
    *,
    ...
    locality_scale_limit = _locality_scale_limit(metrics.asymmetry_rel, config=config)
...
def _next_action_from_groups(
    ...
    if groups.variance_limited:
        return "target paired-seed or control-variate variance reduction for limiting states"
...
def _composite_control_metric_payload(
    ...
    return {
        ...
        "fd_asymmetry_rel": _json_number(metrics.asymmetry_rel),
        "gradient_uncertainty_rel": _json_number(metrics.uncertainty_rel),
        "paired_gradient_uncertainty_rel": _json_number(metrics.paired_uncertainty_rel),
...
def _composite_control_row(
    ...
    *,
    ...
    metrics = _composite_control_metrics(artifact, index=index, path=path, label=label)
...
def _composite_control_rows(
    ...
    *,
    ...
    return [
        ...
        for index, (artifact, path, label) in enumerate(zip(artifacts, paths, labels))
    ]
...
def _composite_launch_plan(
    *,
    ...
    if launch_ready:
        ...
        command_template = (
            "python tools/campaigns/write_vmec_boundary_campaigns.py profile-direction "
...
def _metric_acceptance(
    ...
    uncertainty_ok = (
        metrics.uncertainty_rel is not None
        and metrics.uncertainty_rel <= float(cfg.max_gradient_uncertainty_rel)
    )
...
def _ql_seed_rows(
    ...
    *,
    ...
    if not isinstance(objective_gates, Sequence):
        return []
    context = _ql_seed_artifact_context(artifact, index=index, path=path, label=label)
...
def _ql_screen_state(ql_seed_screen: Mapping[str, Any]) -> _QLScreenState:
    ...
    return _QLScreenState(
        kind=ql_kind,
        passed=ql_passed,
        usable=ql_kind == "nonlinear_turbulence_gradient_ql_seed_screen" and ql_passed,
    ...
    *,
    ...
    if mapping is None:
        return _MappingQuality(
            False, None, None, None, ["missing_state_to_input_mapping"]
        )
    ...
    mapping_passed = (
        bool(mapping.get("passed", False)) or not config.require_mapping_passed
    )
    return _MappingQuality(
...
def _validate_control_variate_campaign_config(
    ...
    if cfg.max_control_mean_pairs < cfg.min_control_mean_pairs:
        raise ValueError(
            "max_control_mean_pairs must be at least min_control_mean_pairs"
        )
...
def _campaign_candidate_values(
    ...
    candidate_pair_count = _finite_int(
        None if candidate is None else candidate.get("n_samples")
    )
...
def _campaign_prediction(
    ...
    *,
    ...
    residual_margin_squared = (
        target_abs_sem * target_abs_sem - residual_sem * residual_sem
    )
    ...
    required = math.ceil(
        ...
        * (abs(beta) * control_sample_std / max(residual_margin_sem, cfg.value_floor))
        ** 2
    )
    independent_control_pairs = max(cfg.min_control_mean_pairs, int(required))
    predicted_control_mean_sem = control_sample_std / math.sqrt(
    ...
    *,
    ...
    return {
        ...
        "summary": {
            ...
            "predicted_combined_sem": _json_number(
                prediction["predicted_combined_sem"]
            ),
        },
...
def nonlinear_gradient_control_variate_campaign_plan(
    ...
    *,
    ...
    blockers = _campaign_initial_blockers(
        candidate, values, value_floor=cfg.value_floor
    )
