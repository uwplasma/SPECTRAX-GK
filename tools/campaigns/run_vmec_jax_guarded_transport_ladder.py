...
DEFAULT_DRIVER = (
    ROOT / "tools" / "campaigns" / "vmec_jax_qa_low_turbulence_optimization.py"
)
...
def _history_with_transport_metric(
    history: dict[str, Any], metric_path: Path | None
) -> dict[str, Any]:
...
def candidate_summary(
    ...
    *,
    ...
    gate_source = (
        "solved_wout_gate.json"
        if gate_path is not None
        else ("reconstructed" if gate else None)
    )
    ...
    return {
        ...
        "gate_checks": {
            name: check.get("passed") for name, check in gate.get("checks", {}).items()
        },
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Passing QA-only candidate directory",
    )
    parser.add_argument(
        "--outdir",
        type=Path,
        default=DEFAULT_OUTDIR,
        help="Transport ladder output root",
    )
    ...
    parser.add_argument(
        ...
        help="Python executable used for candidate runs",
    )
    parser.add_argument(
        "--driver-args",
        default="",
    ...
    parser.add_argument(
        ...
        type=float,
        default=6.0,
        help="Solved-candidate aspect target",
    )
    parser.add_argument(
        "--aspect-atol",
        type=float,
        default=5.0e-2,
        help="Solved-candidate aspect tolerance",
    )
    parser.add_argument(
        "--min-iota",
        type=float,
        default=0.41,
        help="Minimum accepted absolute mean iota",
    )
    parser.add_argument(
        "--qs-max", type=float, default=5.0e-2, help="Maximum accepted QS residual"
    )
    parser.add_argument(
        "--iota-profile-floor",
        type=float,
        default=0.41,
        help="Minimum accepted solved iota profile",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Per-candidate subprocess timeout; 0 disables",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Write the launch plan without running candidates",
    )
    parser.add_argument(
        "--out-json",
        type=Path,
        default=None,
        help="Summary JSON path; defaults inside --outdir",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        bool(args.disable_iota_profile_floor)
        and "--disable-iota-profile-floor" not in driver_args
    ):
    ...
    summaries = [
        candidate_summary(
            ...
            transport_metric_json=Path(args.baseline_metric_json)
            if args.baseline_metric_json is not None
            else None,
        )
    ]
    ...
    for weight in tuple(float(w) for w in args.weights):
        ...
        if not bool(args.dry_run):
            ...
            if not bool(summary.get("passed")) and not bool(
                args.continue_after_failed_gate
            ):
    ...
    transport_admission = build_transport_admission_report(
        summaries, policy=admission_policy
    )
    ...
    payload = {
        ...
        "baseline_metric_json": None
        if args.baseline_metric_json is None
        else str(args.baseline_metric_json),
        ...
        "transport_candidate_admitted": bool(
            transport_admission.get("transport_candidate_admitted")
        ),
    ...
    out_json = (
        ...
        else outdir / "guarded_transport_ladder.json"
    )
