...
def main() -> int:
    ...
    parser.add_argument(
        "--rho-star", type=float, default=CYCLONE_NORMALIZATION.rho_star
    )
    parser.add_argument(
        "--method", type=str, default="rk3", choices=("euler", "rk2", "rk3", "rk4")
    )
    ...
    parser.add_argument(
        "--out", type=Path, default=Path("docs/_static/nonlinear_cyclone_diag.csv")
    )
    ...
    G0 = np.zeros(
        (1, args.Nl, args.Nm, grid.ky.size, grid.kx.size, grid.z.size),
        dtype=np.complex64,
    )
