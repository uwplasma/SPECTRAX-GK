...
def select_candidate(
    rows: Iterable[CandidateRow], *, excluded_cases: set[str]
) -> CandidateRow:
    eligible = [
        row
        for row in rows
...
def resolve_vmec_file(
    raw: str, *, search_roots: Iterable[Path] = DEFAULT_VMEC_SEARCH_ROOTS
) -> Path:
    path = Path(raw)
    if path.exists():
        return path
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--case-slug", default=None, help="Override output slug for the selected case"
    )
    parser.add_argument(
        "--ky",
        type=float,
        default=None,
        help="Override ky instead of using the screen winner",
    )
    parser.add_argument(
        "--horizons",
...
def main(argv: list[str] | None = None) -> int:
    ...
    case_slug = (
        str(args.case_slug) if args.case_slug else _default_case_slug(selected.case)
    )
    ...
    print(
        f"selected {selected.case} ky={ky:.6g} gamma={float(selected.best_gamma):.6g}"
    )
