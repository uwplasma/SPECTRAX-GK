...
def sample_statistics_from_objective_table(
    *,
    ...
    if samples.reduction == "mean":
        weights = np.full(
            objective_table.shape[:-1],
            1.0 / float(np.prod(objective_table.shape[:-1])),
            dtype=float,
        )
        objective_weights = np.full(
            (int(objective_table.shape[-1]),),
            1.0 / float(objective_table.shape[-1]),
            dtype=float,
        )
    else:
        weights = np.asarray(
    ...
    per_sample = np.sum(
        np.asarray(objective_table, dtype=float) * objective_weights, axis=-1
    )
...
def evaluate_metrics(
    args: argparse.Namespace, kinds: tuple[str, ...]
    ...
    base_kind = (
        "quasilinear_flux" if any(kind != "growth" for kind in kinds) else "growth"
    )
    ...
    for kind in kinds:
        ...
        if not np.isfinite(metric):
            raise RuntimeError(
                f"non-finite SPECTRAX transport metric for {kind}: {metric!r}"
            )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Solved VMEC input deck, typically input.final",
    )
    parser.add_argument(
        "--out-json", required=True, type=Path, help="JSON sidecar to write"
    )
    parser.add_argument(
        "--outdir",
        type=Path,
