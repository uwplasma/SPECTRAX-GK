...
def _expected_output(
    out_dir: Path, case: str, horizon: float, grid_label: str, variant: str
) -> Path:
    return (
        out_dir
        / f"{case}_nonlinear_t{_horizon_label(horizon)}_{grid_label}_{variant}.out.nc"
    )
...
def _state_ensemble_command(
    *,
    ...
    inputs.append(
        _expected_output(state_out_dir, state_case, tmax, grid_label, dt_variant_label)
    )
    ...
    for seed, seed_dt in seed_dt_variants:
        ...
        inputs.append(
            _expected_output(state_out_dir, state_case, tmax, grid_label, label)
        )
    ...
    output_gate_command = (
        f"{PYTHON_CMD} tools/release/check_nonlinear_transport_gates.py runtime-outputs "
        + " ".join(_repo_relative(path) for path in inputs)
...
def _promotion_contract(
    *,
    ...
    fd_prefix = (
        ROOT / "docs" / "_static" / f"{case}_{parameter_name}_central_fd_gradient_gate"
    )
    fd_json = fd_prefix.with_suffix(".json")
    evidence_json = (
        ROOT / "docs" / "_static" / "nonlinear_turbulence_gradient_evidence_status.json"
    )
    gap_json = (
        ROOT
        / "docs"
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest = {
        ...
        "state_vmec_files": {
            state: _repo_relative(path) for state, path in state_vmec.items()
        },
        "vmec_file_preflight": {
            "vmec_files_exist": True,
            "vmec_paths_distinct": True,
            "vmec_contents_distinct": len(
                {row["sha256"] for row in state_file_metadata.values()}
            )
        ...
        "run_contract": {
            ...
            "joint_seed_timestep_replicates": [
                f"seed{seed}_dt{_float_label(seed_dt)}"
                for seed, seed_dt in seed_dt_variants
            ],
        },
    ...
    print(
        json.dumps(
            {"configs": total_configs, "manifest": _repo_relative(manifest_path)},
            indent=2,
            sort_keys=True,
        )
    )
