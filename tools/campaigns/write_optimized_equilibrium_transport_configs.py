...
def _expected_output(
    ...
    horizon_label = (
        str(int(horizon))
        if abs(horizon - int(horizon)) < 1e-12
        else str(horizon).replace(".", "p")
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Concrete optimized-equilibrium VMEC wout file.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.64,
        help="VMEC toroidal-flux label for the flux tube.",
    )
    parser.add_argument(
        "--alpha",
        type=float,
        default=0.0,
        help="VMEC/Boozer field-line label for the flux tube.",
    )
    parser.add_argument(
        "--npol",
        type=float,
        default=1.0,
        help="Number of poloidal turns in the VMEC flux tube.",
    )
    parser.add_argument(
        "--tprim", type=float, default=3.0, help="Ion temperature-gradient drive."
    )
    parser.add_argument(
        "--fprim", type=float, default=1.0, help="Ion density-gradient drive."
    )
    parser.add_argument(
        "--nu",
        type=float,
        default=0.01,
        help="Collision frequency used by the nonlinear transport audit.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(grids) != 1:
        raise ValueError(
            "optimized-equilibrium promotion configs currently require exactly one grid"
        )
    ...
    dt_variant_label = f"dt{float(args.dt_variant):.12g}".replace(".", "p").replace(
        "-", "m"
    )
    ...
    print(
        json.dumps(
            {"configs": len(written), "manifest": _repo_relative(manifest_path)},
            indent=2,
            sort_keys=True,
        )
    )
