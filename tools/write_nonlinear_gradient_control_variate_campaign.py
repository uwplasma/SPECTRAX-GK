...
DEFAULT_OUT_PREFIX = ROOT / "docs" / "_static" / "nonlinear_gradient_control_variate_campaign_plan"
...
def _plot(path: Path, report: dict[str, Any]) -> None:
    ...
    axes[0].bar(np.arange(len(labels)), values, color=["#4c78a8", "#54a24b", "#f58518"], edgecolor="0.25")
    ...
    title = str(report.get("action", "control variate campaign")).replace("_", " ").capitalize()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--case", default="nonlinear_gradient_control_variate_campaign_plan")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"action": report["action"], "json": _repo_relative(json_path)}, indent=2, sort_keys=True))
    return 0
