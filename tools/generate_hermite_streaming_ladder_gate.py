...
def build_hermite_streaming_ladder_gate(
    *,
    ...
    ladder_sharded = hermite_streaming_ladder_shard_map(state, plan, vth=float(vth), devices=device_list)
    reduction_reference = velocity_field_reduce_reference(state, axis="hermite")
    reduction_sharded = velocity_field_reduce_shard_map(state, plan, axis="hermite", devices=device_list)
    ...
    ladder_rel = ladder_abs / jnp.maximum(ladder_scale, jnp.asarray(1.0e-30, dtype=ladder_scale.dtype))
    ...
    identity_passed = bool(max_ladder_abs_error <= float(atol) and max_ladder_rel_error <= float(rtol))
