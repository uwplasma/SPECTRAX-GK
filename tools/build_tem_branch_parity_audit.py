...
REFERENCE_PROVENANCE = (
    ...
    "tools/digitize_reference.py; this is not a direct GX/SPECTRAX-GK dump."
)
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            ...
            for column in REQUIRED_COLUMNS:
                ...
                if value is None:
                    raise ValueError(f"{path} contains non-finite {column!r}: {row.get(column)!r}")
                parsed[column] = value
            rows.append(parsed)
    if not rows:
...
def build_audit_payload(
    *,
    ...
    if np.any(safe_omega):
        ...
        worst_safe_rel_omega_ky = float(ky[safe_omega][int(np.argmax(np.abs(rel_omega[safe_omega])))])
    else:
...
def write_artifacts(payload: dict[str, Any], *, out_png: Path = DEFAULT_OUT) -> dict[str, str]:
    ...
    ax = axes[1, 0]
    width = 0.017
    ax.bar(ky - width / 2, gamma_error, width=width, label=r"$|\Delta\gamma|$", color="#2a9d8f")
    ax.bar(ky + width / 2, omega_error, width=width, label=r"$|\Delta\omega|$", color="#e9c46a")
    ax.set_xlabel(r"$k_y \rho_i$")
