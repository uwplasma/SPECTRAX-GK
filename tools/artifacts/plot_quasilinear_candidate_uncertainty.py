...
DEFAULT_CANDIDATE_OUT = ROOT / "docs/_static/quasilinear_candidate_uncertainty.png"
DEFAULT_REGULARIZATION_OUT = (
    ROOT / "docs/_static/quasilinear_candidate_regularization_sweep.png"
)
...
def _spectral_envelope_feature_vector(
    case: SaturationCase, *, floor: float
) -> np.ndarray:
    ...
    positive_ky_centroid = float(
        np.sum(ky * positive_weight) / max(float(np.sum(positive_weight)), floor)
    )
    if np.sum(weight) > floor:
        ...
        ky_weighted_std = math.sqrt(
            max(
                float(
                    np.sum(weight * (ky - weight_mean_ky) ** 2) / float(np.sum(weight))
                ),
                0.0,
            )
        )
...
def _state_feature_matrix(
    cases: tuple[SaturationCase, ...], *, floor: float
) -> np.ndarray:
    return np.vstack([_state_feature_vector(case, floor=floor) for case in cases])


def _spectral_envelope_feature_matrix(
    cases: tuple[SaturationCase, ...], *, floor: float
) -> np.ndarray:
    return np.vstack(
        [_spectral_envelope_feature_vector(case, floor=floor) for case in cases]
    )
...
def _ridge_loglinear_holdout_row(
    *,
    ...
    rel_error = abs(predicted - holdout_observed) / max(
        abs(holdout_observed), observed_floor
    )
    return {
...
def _candidate_holdout_payload(task: dict[str, Any]) -> dict[str, Any]:
    ...
    null_error = abs(null_predicted - holdout_observed) / max(
        abs(holdout_observed), observed_floor
    )
    null_row = {
    ...
    for candidate in candidates:
        ...
        if candidate == "spectral_envelope_ridge":
            if envelope_features_all is None:
                raise AssertionError(
                    "spectral_envelope_ridge requires precomputed features"
                )
            candidate_rows[candidate] = _ridge_loglinear_holdout_row(
                case=holdout_case,
                train_cases=train_cases,
        ...
        train_residual = np.log(
            (train_observed + observed_floor)
            / np.maximum(train_predicted, observed_floor)
        )
        residual_mean = float(np.mean(train_residual))
        residual_sigma = (
            float(np.std(train_residual, ddof=1)) if train_residual.size > 1 else 0.0
        )
        ...
        if lo > hi:
            lo, hi = hi, lo
        rel_error = abs(holdout_predicted - holdout_observed) / max(
            abs(holdout_observed), observed_floor
        )
...
def build_candidate_uncertainty_report(
    ...
    *,
    ...
    features_all = (
        _state_feature_matrix(cases, floor=observed_floor)
        if "linear_state_ridge" in candidates
        else None
    )
    ...
    holdout_payloads = sorted(
        holdout_payloads, key=lambda payload: int(payload["holdout_idx"])
    )
    ...
    null_errors = np.asarray(
        [row["absolute_relative_error"] for row in null_rows], dtype=float
    )
    ...
    for candidate, rows in candidate_rows.items():
        ...
        coverage = float(
            np.mean(
                [bool(row["prediction_interval_contains_observed"]) for row in rows]
            )
        )
        mean_error = float(np.nanmean(errors))
        promotion_eligible = all(
            bool(row.get("promotion_eligible", True)) for row in rows
        )
    ...
    for candidate, payload in candidates_report.items():
        ...
        beats_linear = (
            linear_mean is None
            or candidate == "linear_weight"
            or mean_error < linear_mean
        )
        if (
...
def write_candidate_uncertainty_figure(
    ...
    *,
    ...
    observed_all = np.asarray(
        [row["observed_heat_flux"] for row in null_rows], dtype=float
    )
    positive = [observed_all[observed_all > 0.0]]
    for payload in candidates.values():
        pred = np.asarray(
            [row["predicted_heat_flux"] for row in payload["rows"]], dtype=float
        )
        lo = np.asarray(
            [row["prediction_interval_low"] for row in payload["rows"]], dtype=float
        )
        hi = np.asarray(
            [row["prediction_interval_high"] for row in payload["rows"]], dtype=float
        )
    ...
    for candidate, payload in candidates.items():
        ...
        predicted = np.asarray(
            [row["predicted_heat_flux"] for row in rows], dtype=float
        )
        low = np.asarray([row["prediction_interval_low"] for row in rows], dtype=float)
        high = np.asarray(
            [row["prediction_interval_high"] for row in rows], dtype=float
        )
        yerr = np.vstack(
            [np.maximum(predicted - low, 0.0), np.maximum(high - predicted, 0.0)]
        )
        ax0.errorbar(
            observed,
            predicted,
    ...
    mean_errors.append(
        float(report["null_training_mean_baseline"]["mean_abs_relative_error"])
    )
    ...
    for xpos, err, cov, eligible in zip(
        x, mean_errors, coverages, promotion_eligible, strict=True
    ):
        text = (
            f"{err:.2g}" if not math.isfinite(cov) else f"{err:.2g}\ncoverage {cov:.2g}"
        )
        if not eligible:
            text = f"{err:.2g}\nineligible"
        ax1.text(
...
def score_regularization_sweep(
    ...
    *,
    ...
    """Score spectral-envelope ridge regularization using leave-one-out rows."""
    ...
    for lambda_value in lambdas:
        ...
        holdout_mask = np.asarray(
            [case.split == "holdout" for case in cases], dtype=bool
        )
        coverage = float(
            np.mean(
                [bool(row["prediction_interval_contains_observed"]) for row in loo_rows]
            )
        )
        mean_error = float(np.mean(errors))
        holdout_mean_error = (
            float(np.mean(errors[holdout_mask])) if np.any(holdout_mask) else mean_error
        )
        row = {
            ...
            "promotion_eligible": bool(
                all(bool(item.get("promotion_eligible", True)) for item in loo_rows)
            ),
    ...
    return {
        ...
        "best_holdout_mean_abs_relative_error": float(
            best["holdout_mean_abs_relative_error"]
        ),
        "best_prediction_interval_coverage": float(
            best["prediction_interval_coverage"]
        ),
        "promotion_gate": {
            "passed": bool(accepted),
            ...
            "blockers": []
            if accepted
            else ["best_regularization_transport_error_above_gate"],
        },
        "notes": (
...
def write_regularization_sweep_figure(
    report: dict[str, Any],
    *,
    out: str | Path = DEFAULT_REGULARIZATION_OUT,
    ...
    holdout = np.asarray(
        [row["holdout_mean_abs_relative_error"] for row in rows], dtype=float
    )
    max_error = np.asarray([row["max_abs_relative_error"] for row in rows], dtype=float)
    coverage = np.asarray(
        [row["prediction_interval_coverage"] for row in rows], dtype=float
    )
    ...
    ax0.axhline(
        gate, color="#c2410c", linestyle="--", linewidth=1.4, label=f"{gate:.2f} gate"
    )
    ...
    ax1.axhline(
        ...
        linestyle="--",
        linewidth=1.4,
        label=f"coverage gate {cov_gate:.2f}",
    )
...
def build_candidate_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--out",
        default=str(DEFAULT_CANDIDATE_OUT),
    )
    ...
    parser.add_argument(
        "--workers", type=int, default=1, help="Independent holdout workers."
    )
    parser.add_argument(
...
def build_regularization_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Audit spectral-envelope ridge regularization for QL candidates."
    )
    parser.add_argument("--out", type=Path, default=DEFAULT_REGULARIZATION_OUT)
    parser.add_argument(
        "--title", default="Quasilinear spectral-envelope regularization audit"
    )
...
def _run_candidate_uncertainty(args: argparse.Namespace) -> int:
    ...
    print(
        "promotion_gate_passed={passed} accepted_candidates={accepted}".format(
            ...
            accepted=",".join(report["promotion_gate"]["accepted_candidates"])
            or "none",
        )
    )
    return 0
...
def _run_regularization_sweep(args: argparse.Namespace) -> int:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if raw_argv and raw_argv[:1] == ["regularization-sweep"]:
        return _run_regularization_sweep(
            build_regularization_parser().parse_args(raw_argv[1:])
        )
    return _run_candidate_uncertainty(build_candidate_parser().parse_args(raw_argv))
