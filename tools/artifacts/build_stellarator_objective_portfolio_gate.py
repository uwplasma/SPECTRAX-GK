...
def _fixture_rows(params: jnp.ndarray) -> jnp.ndarray:
    ...
    growth = (
        ...
        + 0.19 * ripple**2
        + 0.035 * ky
        + 0.018 * jnp.cos(alpha + shear)
        + 0.010 * surface
    )
    ql_flux = growth * (0.80 + 0.35 * pressure**2 + 0.12 * ky) / (0.55 + ky)
    window_flux = (
        ql_flux * (1.0 + 0.08 * jnp.sin(alpha) + 0.18 * surface)
        + 0.015 * ripple * pressure
    )
...
def stellarator_objective_portfolio_gate_payload(
    *,
    ...
    normalized_objective_weights = portfolio_objective_weight_vector(
    ...
    step = float(fd_step)
    finite_difference = (
        scalar_objective(params + step * direction)
        - scalar_objective(params - step * direction)
    ) / (2.0 * step)
                ...
                samples.append(
                    {
                        ...
                        "ky": ky_label,
                        "sample_weight": float(
                            sample_weights_np[surface_index, alpha_index, ky_index]
                        ),
                        "scalar_value": float(
                            sample_scalars_np[surface_index, alpha_index, ky_index]
                        ),
                        **{
    ...
    return {
        ...
        "normalized_objective_weights": np.asarray(
            normalized_objective_weights, dtype=float
        ).tolist(),
        "value": float(value),
...
def write_stellarator_objective_portfolio_artifacts(
    ...
    *,
    ...
    sample_scalars = np.asarray(
        [
            float(row.get("scalar_value", np.nan))
            for row in rows
            if isinstance(row, dict)
        ],
        dtype=float,
    ...
    sample_weights = np.asarray(
        [
            float(row.get("sample_weight", np.nan))
            for row in rows
            if isinstance(row, dict)
        ],
        dtype=float,
    ...
    colors = plt.cm.viridis(
        sample_weights / max(float(np.max(sample_weights)), 1.0e-12)
    )
    ...
    ax_derivatives.bar(
        derivative_names, derivative_values, color=["#219ebc", "#2a9d8f", "#f4a261"]
    )
