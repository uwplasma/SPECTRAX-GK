    ...
    try:
        with Dataset(path, "r") as root:
            if "Grids" in root.groups and "time" in root.groups["Grids"].variables:
                time = np.asarray(
                    root.groups["Grids"].variables["time"][:], dtype=float
                )
...
def classify_record(
    record: dict[str, Any], *, min_transport_time: float = MIN_TRANSPORT_TIME
) -> str:
    ...
    tmax = (
        _finite_float(
            record.get("effective_tmax"), _finite_float(record.get("tmax"), 0.0)
        )
        or 0.0
    )
    threshold_tmax = (
        _max_float(record.get("effective_tmax"), record.get("source_tmax")) or tmax
    )
    ...
    if gate_passed and (
        tmax >= min_transport_time or threshold_tmax >= min_transport_time
    ):
...
def production_optimization_blockers(
    ...
    *,
    ...
    if record_status == "reduced_estimator_not_transport_average":
        ...
        blockers.append(
            "long nonlinear trace lacks a passed convergence/reference gate"
        )
        ...
        blockers.append(
            "missing passed post-transient nonlinear transport-average gate"
        )
...
def _pilot_records(root: Path) -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
    for relative in [
        "docs/_static/external_vmec_dshape_nonlinear_t150_n32_pilot.json",
        "docs/_static/external_vmec_dshape_nonlinear_t150_n48_pilot.json",
        "docs/_static/external_vmec_dshape_nonlinear_t150_n64_pilot.json",
        "docs/_static/external_vmec_dshape_nonlinear_t250_n48_pilot.json",
        "docs/_static/external_vmec_dshape_nonlinear_t250_n64_pilot.json",
        "docs/_static/external_vmec_itermodel_nonlinear_t150_n48_pilot.json",
        "docs/_static/external_vmec_itermodel_nonlinear_t150_n64_pilot.json",
        "docs/_static/external_vmec_itermodel_nonlinear_t250_n48_pilot.json",
        "docs/_static/external_vmec_itermodel_nonlinear_t250_n64_pilot.json",
        "docs/_static/external_vmec_itermodel_nonlinear_t350_n48_pilot.json",
        "docs/_static/external_vmec_itermodel_nonlinear_t350_n64_pilot.json",
        "docs/_static/external_vmec_updown_asym_nonlinear_t450_n48_pilot.json",
        "docs/_static/external_vmec_updown_asym_nonlinear_t450_n64_pilot.json",
        ...
        records.append(
            {
                ...
                "effective_tmax": _max_float(
                    payload.get("tmax"), _source_tmax(root, payload.get("source"))
                ),
                "tmax": _max_float(
                    payload.get("tmax"), _source_tmax(root, payload.get("source"))
                ),
    ...
    for relative in [
        "docs/_static/external_vmec_dshape_grid_convergence_gate.json",
        "docs/_static/external_vmec_dshape_high_grid_convergence_gate.json",
        "docs/_static/external_vmec_dshape_t250_high_grid_convergence_gate.json",
        "docs/_static/external_vmec_circular_t150_high_grid_convergence_gate.json",
        "docs/_static/external_vmec_circular_t250_high_grid_convergence_gate.json",
        "docs/_static/external_vmec_itermodel_t150_high_grid_convergence_gate.json",
        "docs/_static/external_vmec_itermodel_t250_high_grid_convergence_gate.json",
...
def _startup_and_reduced_records(root: Path) -> list[dict[str, Any]]:
    specs = [
        (
            ...
            "Compact nonlinear FD startup audit",
        ),
        (
            "docs/_static/vmec_boozer_nonlinear_window_fd_audit.json",
            "VMEC/Boozer nonlinear FD startup audit",
        ),
        (
            "docs/_static/vmec_boozer_nonlinear_window_gradient_gate.json",
            "QH reduced nonlinear-window estimator gate",
        ),
        (
            "docs/_static/vmec_boozer_li383_nonlinear_window_gradient_gate.json",
            "Li383 reduced nonlinear-window estimator gate",
        ),
        (
            "docs/_static/stellarator_itg_nonlinear_optimization.json",
            "Reduced stellarator nonlinear objective example",
        ),
    ]
    records: list[dict[str, Any]] = []
        ...
        records.append(
            {
                ...
                "notes": str(
                    payload.get(
                        "next_action", "not a promoted nonlinear transport average"
                    )
                ),
            }
        )
...
def build_payload(
    root: Path = ROOT, *, min_transport_time: float = MIN_TRANSPORT_TIME
) -> dict[str, Any]:
    """Build the nonlinear transport time-horizon audit payload."""

    root = Path(root)
    records = (
        _release_gate_records(root)
        + _pilot_records(root)
        + _startup_and_reduced_records(root)
    )
    for record in records:
        record["status"] = classify_record(
            record, min_transport_time=min_transport_time
        )
    ...
    return {
        ...
        "min_transport_time": float(min_transport_time),
        "production_optimization_requirements": list(
            PRODUCTION_OPTIMIZATION_REQUIREMENTS
        ),
        "summary": {
            "n_records": len(records),
            "status_counts": counts,
            "release_transport_gate_passed": counts.get(
                "release_transport_gate_passed", 0
            ),
            "short_or_reduced_not_transport": counts.get(
                "short_or_startup_not_transport_average", 0
            )
...
def write_artifacts(
    ...
    colors = [
        STATUS_COLORS.get(
            str(record["status"]), STATUS_COLORS["missing_or_unclassified"]
        )
        for record in records
    ]
    ...
    ax.axvline(
        float(payload["min_transport_time"]),
        color="#111827",
        linestyle="--",
        linewidth=1.7,
    )
    ...
    handles = [
        Patch(
            facecolor=STATUS_COLORS.get(
                status, STATUS_COLORS["missing_or_unclassified"]
            ),
