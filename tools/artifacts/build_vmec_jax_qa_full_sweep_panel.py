...
DEFAULT_STRICT_AUDIT_ROOT = (
    ROOT / "docs" / "_static" / "optimized_equilibrium_replicates"
)
...
def _sample_set_fingerprint(setup: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "objective_scale": _finite_float(
            config.get("objective_scale"), default=math.nan
        ),
...
def _optimizer_comparison_metadata(
    *,
    ...
    transport_kind = (
        None
        if constraints_only
        else setup.get("transport_kind", history.get("transport_metric_kind"))
    )
    metadata = {
        "case_id": case_id,
        ...
        "transport_metric_final": _finite_float(
            history.get(
                "transport_metric_final", history.get("transport_objective_final")
            )
        ),
...
def _load_iota_profile_from_wout(wout_path: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        s_iotas = (
            np.linspace(0.0, 1.0, int(iotas.size)) if iotas.size else np.asarray([])
        )
        s_iotaf = (
            np.linspace(0.0, 1.0, int(iotaf.size)) if iotaf.size else np.asarray([])
        )
        return {
...
def _admitted_authoritative_rerun_wout(
    ...
    *,
    ...
    if wout_repro_gate_passed is not False:
        return None
    if rerun_wout_admission_gate is None or not bool(
        rerun_wout_admission_gate.get("passed", False)
    ):
        return None
...
def _strict_full_sweep_audit_status(
    audit_root: Path = DEFAULT_STRICT_AUDIT_ROOT,
) -> dict[str, Any]:
    ...
    return {
        ...
        "n_empty_window_ensembles": sum(
            1 for row in rows if int(row["n_finite_means"]) == 0
        ),
        "rows": rows,
    }
...
def _audit_case_token(case_id: str) -> str:
    return "vmec_qa_full_sweep_" + case_id.replace("/", "_").replace(".", "p").replace(
        "-", "m"
    ).replace("+", "p")


def _nonlinear_audit_command(
    wout_path: Path | None, case_id: str, *, run_completed: bool
) -> str | None:
...
def _row_from_run(
    root: Path, *, campaign_root: Path, runs_root: Path
) -> dict[str, Any]:
    ...
    authoritative_wout = (
        authoritative_rerun_wout if authoritative_rerun_wout is not None else wout_path
    )
    ...
    return {
        ...
        "authoritative_wout": _repo_relative(authoritative_wout)
        if authoritative_wout.exists()
        else None,
...
def _selected_rows(
    rows: list[dict[str, Any]], *, max_count: int = 4
) -> list[dict[str, Any]]:
    ...
    for case_id in PREFERRED_SURFACE_IDS:
        ...
        if (
            ...
            and bool(row.get("has_wout_final"))
        ):
            selected.append(row)
    for row in rows:
...
def _plot_summary_table(ax: plt.Axes, rows: list[dict[str, Any]]) -> None:
    ax.axis("off")
    lines = [
        "case                         A      iota      QS        transport       status"
    ]
    for row in rows[:8]:
        hist = row["history"]
        ...
        transport_text = (
            f"{transport:9.2e}" if math.isfinite(transport) else "    n/a  "
        )
...
def _plot_objective_history(ax: plt.Axes, rows: list[dict[str, Any]]) -> None:
    for row in rows:
        ...
        ax.semilogy(
            ...
            np.maximum(series / abs(y0), 1.0e-16),
            lw=1.9,
            label=row["label"],
        )
...
def _plot_metric_bars(ax: plt.Axes, rows: list[dict[str, Any]]) -> None:
    ...
    transport = np.asarray(
        [row["history"]["transport_metric_final"] for row in rows], dtype=float
    )
...
def _plot_iota_profiles(ax: plt.Axes, rows: list[dict[str, Any]]) -> None:
    ...
    if plotted:
        ...
        ax.text(
            0.5, 0.5, "No WOUT iota profiles available yet", ha="center", va="center"
        )
...
def _plot_transport_bars(ax: plt.Axes, rows: list[dict[str, Any]]) -> None:
    ...
    if not transport_rows:
        ...
        ax.text(
            0.5,
            0.5,
            "No reduced transport metric completed yet",
            ha="center",
            va="center",
        )
        ...
        return
    values = np.asarray(
        [row["history"]["transport_metric_final"] for row in transport_rows],
        dtype=float,
    )
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [row["label"] for row in transport_rows], rotation=35, ha="right", fontsize=7
    )
    ax.set_yscale(
        "log"
        if np.nanmax(values) / max(np.nanmin(values), 1.0e-300) > 100.0
        else "linear"
    )
...
def _plot_q_traces(
    ax: plt.Axes, rows: list[dict[str, Any]], *, audit_status: str
) -> None:
    plotted = False
    ...
    if plotted:
        ...
        if audit_status == "failed_empty_strict_window":
            ...
            message = f"Strict audit harvested, not promoted:\n{audit_status}."
        else:
            message = (
...
def plot_payload(payload: dict[str, Any], out: Path) -> None:
    ...
    if str(status).startswith("failed"):
        ...
        status_clause = (
            "reduced objectives are diagnostics until matched nonlinear audits exist"
        )
    fig.suptitle(
...
def main() -> int:
    ...
    if not payload["cases"]:
        raise FileNotFoundError(
            f"no optimizer history.json files found under {args.run_root}"
        )
        ...
        json.dumps(
            {
                ...
                "cases_with_q_traces": payload["summary"][
                    "n_cases_with_nonlinear_q_traces"
                ],
                "nonlinear_transport_audit_status": payload["summary"][
