...
DEFAULT_CASES: tuple[ParityCase, ...] = (
    ...
    ParityCase(
        "nfp3_QI_fixed_resolution_final", "QI fixed resolution", "quasi-isodynamic", 8
    ),
    ParityCase(
        "shaped_tokamak_pressure", "shaped tokamak", "axisymmetric finite-beta", 8
    ),
)
...
DEFAULT_QI_VARIANTS: tuple[ParityCase, ...] = (
    ...
    ParityCase(
        "nfp4_QI_finite_beta",
        "QI finite-beta input",
        "quasi-isodynamic input variant",
        8,
    ),
)
...
def _sample_set_provenance(
    *,
    ...
    unique_sample_set_ids = {str(entry["sample_set_id"]) for entry in all_entries}
    return {
...
def _vmec_example_artifacts(
    ...
    try:
        ...
        return (
            None,
            None,
            f"vmec_jax_example_paths_unavailable: {type(exc).__name__}: {exc}",
        )
...
def _rejection_row(
    ...
    *,
    ...
    mode_floor_passed = (
        int(case.mboz) >= MIN_BOOZER_MODE_COUNT
        and int(case.nboz) >= MIN_BOOZER_MODE_COUNT
    )
    return {
        "validation_scope": "qi_seed_robustness",
        "sample_set_id": _case_sample_set_id(case),
...
def _row_from_report(case: ParityCase, report: dict[str, object]) -> dict[str, object]:
    ...
    row = {
        ...
        "mboz": mboz,
        "nboz": nboz,
        "mode_floor_passed": mboz >= MIN_BOOZER_MODE_COUNT
        and nboz >= MIN_BOOZER_MODE_COUNT,
        "available": available,
        ...
        "equal_arc_core_worst_scalar_rel": _finite_float(
            report.get("equal_arc_core_worst_scalar_rel")
        ),
        ...
        "equal_arc_core_tolerance": _finite_float(
            report.get("equal_arc_core_tolerance")
        ),
        "equal_arc_derivative_tolerance": _finite_float(
            report.get("equal_arc_derivative_tolerance")
        ),
        "equal_arc_metric_tolerance": _finite_float(
            report.get("equal_arc_metric_tolerance")
        ),
        "equal_arc_drift_tolerance": _finite_float(
            report.get("equal_arc_drift_tolerance")
        ),
        "equal_arc_core_passed": bool(report.get("equal_arc_core_passed", False)),
        "equal_arc_bgrad_passed": bool(
            report.get("equal_arc_derivative_passed", False)
        ),
        ...
        "worst_core_normalized_max_abs": _finite_float(
            report.get("worst_core_normalized_max_abs")
        ),
...
def _qi_row_from_evaluated_row(
    case: ParityCase, row: dict[str, object]
) -> dict[str, object]:
    qi_row = dict(row)
    qi_row.update(
        {
...
def _qi_rejection_from_unavailable_row(
    case: ParityCase, row: dict[str, object]
) -> dict[str, object]:
    detail = (
        row.get("reason") or row.get("error") or "mode-21 QI report was unavailable"
    )
...
def build_qi_seed_robustness(
    ...
    *,
    ...
    for case in variants:
        ...
        if (
            int(case.mboz) < MIN_BOOZER_MODE_COUNT
            or int(case.nboz) < MIN_BOOZER_MODE_COUNT
        ):
            rows.append(
                _rejection_row(
                    case,
        ...
        if _can_reuse_row(case, reusable):
            rows.append(
                _qi_row_from_evaluated_row(case, cast(dict[str, object], reusable))
            )
            continue
    ...
    failed = [
        row for row in evaluated if not bool(row.get("qi_validation_passed", False))
    ]
...
def build_parity_matrix(
    ...
    *,
    ...
    all_equal_arc_passed = all_available and all(
        bool(row["equal_arc_all_passed"]) for row in rows
    )
    ...
    return {
        ...
        "summary": {
            ...
            "n_equal_arc_passed": sum(
                1 for row in rows if bool(row["equal_arc_all_passed"])
            ),
            "all_available": all_available,
            "all_equal_arc_passed": all_equal_arc_passed,
            "qi_seed_robust_gate_passed": bool(
                cast(dict[str, object], qi_seed_robustness["summary"]).get(
                    "seed_robust_gate_passed", False
                )
            ),
        },
        "notes": (
...
def _metric_arrays(
    ...
    for i, row in enumerate(rows):
        for j, (metric_key, tolerance_key, pass_key) in enumerate(
            zip(metric_keys, tolerance_keys, pass_keys, strict=True)
        ):
...
def write_parity_matrix_artifacts(
    payload: dict[str, object], *, out: str | Path = DEFAULT_OUT
) -> dict[str, str]:
    ...
    cmap.set_bad("#e5e7eb")
    im = ax.imshow(
        masked, cmap=cmap, norm=LogNorm(vmin=1.0e-3, vmax=vmax), aspect="auto"
    )
