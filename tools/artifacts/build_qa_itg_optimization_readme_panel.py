...
DEFAULT_GEOMETRY_PNG = (
    ROOT / "docs/_static/vmec_jax_qa_solved_boundary_boozer_panel.png"
)
...
DEFAULT_LANDSCAPE_CSV = (
    ROOT / "docs/_static/vmec_boundary_transport_landscape_rbc11_full.csv"
)
DEFAULT_ADMISSION_JSON = (
    ROOT / "docs/_static/vmec_boundary_transport_landscape_rbc11_full_admission.json"
)
...
def _standard_errors_from_landscape(
...
def _normalize(
    values: np.ndarray, errors: np.ndarray, baseline_index: int
...
def _plot_wout_boozer(ax: plt.Axes, wout_path: Path, title: str) -> None:
    ...
    contour = ax.contour(
        zeta_grid, theta_grid, bmag, levels=levels, cmap="jet", linewidths=0.75
    )
    ax.set_title(title, fontsize=9.2, pad=2)
...
def _plot_iota(ax: plt.Axes, sweep: dict[str, Any]) -> None:
    cases = [
        ...
        (
            "projected_guarded_ladder/transport_weight_0p001",
            "projected opt.",
            "#c2410c",
            "-",
        ),
...
def _plot_landscape(
    ax: plt.Axes, rows: list[dict[str, float | str]], payload: dict[str, Any]
) -> None:
    ...
    for kind, (label, color, marker) in styles.items():
        if not any(kind in row for row in rows):
            continue
        y = np.asarray(
            [float(row.get(kind, float("nan"))) for row in rows], dtype=float
        )
        err_lookup = _standard_errors_from_landscape(payload, kind)
        err = np.asarray(
            [err_lookup.get(str(row["label"]), np.nan) for row in rows], dtype=float
        )
        if not np.any(np.isfinite(err)):
            err = np.zeros_like(y)
        yn, en = _normalize(y, err, baseline_index)
        ax.errorbar(
            x, yn, yerr=en, marker=marker, color=color, lw=2.0, capsize=3, label=label
        )
    ...
    ax.set_title(
        "Linear and quasilinear boundary landscape", loc="left", fontweight="bold"
    )
...
def _plot_replicated_landscape(ax: plt.Axes, admission: dict[str, Any] | None) -> None:
    ...
    means = [float(baseline["ensemble_mean"])] + [
        float(row["ensemble_mean"]) for row in candidates
    ]
    sems = [float(baseline["combined_sem"])] + [
        float(row["combined_sem"]) for row in candidates
    ]
    colors = ["#334155"] + [
        "#0f766e" if bool(row.get("admitted", False)) else "#b91c1c"
        for row in candidates
    ]
    xpos = np.arange(len(labels))
    ax.bar(
        xpos, means, yerr=sems, color=colors, edgecolor="0.15", linewidth=0.5, capsize=4
    )
...
def _plot_q_traces(
    ax: plt.Axes, sweep: dict[str, Any], matched: dict[str, Any]
) -> None:
    cases = [
...
def build_panel(
    *,
    ...
    ax_geom_title.set_title(
        "Solved QA geometry and LCFS |B|", loc="left", fontweight="bold", pad=4
    )
    ...
    try:
        ...
        candidate_wout = _wout_path_from_case(
            sweep, "projected_guarded_ladder/transport_weight_0p001"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        json.dumps(
            {"output": sidecar["output"], "sources": sidecar["sources"]},
            indent=2,
            sort_keys=True,
        )
    )
