...
def plot_payload(payload: dict[str, Any], out: Path) -> None:
    ...
    for offset, design in zip(
        ...
        ax_final.bar(
            x + offset, values, width=width, color=COLORS[name], label=LABELS[name]
        )
...
def main() -> int:
    ...
    payload = qa_low_turbulence_comparison_payload(
        cfg, finite_difference_workers=int(args.workers)
    )
    paths = write_artifacts(payload, args.out, write_pdf=bool(args.pdf))
    print(
        json.dumps(
            {"passed": payload["comparison_metrics"]["passed"], "paths": paths},
            indent=2,
        )
    )
