...
DEFAULT_OUT = (
    ROOT / "docs" / "_static" / "vmec_jax_qa_transport_optimization_status.png"
)
...
DEFAULT_LINE_SEARCH_JSON = (
    ROOT / "docs" / "_static" / "vmec_boozer_aggregate_line_search_comparison.json"
)
DEFAULT_QL_RULE_JSON = (
    ROOT / "docs" / "_static" / "quasilinear_saturation_rule_sweep.json"
)
DEFAULT_QL_MODEL_JSON = (
    ROOT / "docs" / "_static" / "quasilinear_model_selection_status.json"
)
DEFAULT_NONLINEAR_AUDIT_JSON = (
    ROOT / "docs" / "_static" / "qa_no_ess_to_optimized_nonlinear_audit.json"
)
DEFAULT_LANDSCAPE_ADMISSION_JSON = (
    ROOT / "docs" / "_static" / "vmec_boundary_transport_landscape_admission.json"
)
DEFAULT_POSITIVE_PRELAUNCH_JSON = (
    ROOT / "docs" / "_static" / "vmec_boundary_transport_prelaunch_gate.json"
)
DEFAULT_NEGATIVE_PRELAUNCH_JSON = (
    ROOT / "docs" / "_static" / "strict_qa_top12_edge_prelaunch_gate.json"
)
DEFAULT_CAMPAIGN_ADMISSION_JSON = (
    ROOT / "docs" / "_static" / "nonlinear_campaign_admission_report.json"
)
EXPECTED_NONLINEAR_AUDIT_CLAIM_LEVEL = (
    "matched_baseline_to_optimized_replicated_nonlinear_transport_audit"
)
...
def _candidate_row(label: str, root: Path, *, objective_kind: str) -> dict[str, Any]:
    ...
    wout_repro_gate = (
        _read_json(wout_repro_gate_path) if wout_repro_gate_path.exists() else None
    )
    rerun_wout_admission_gate_path = root / "rerun_wout_admission_gate.json"
    rerun_wout_admission_gate = (
        _read_json(rerun_wout_admission_gate_path)
        if rerun_wout_admission_gate_path.exists()
        else None
    )
    ...
    def check_margin(name: str) -> float:
        ...
        if not isinstance(item, dict):
            return math.nan
        if name == "aspect":
            return _finite_float(item.get("absolute_tolerance")) - _finite_float(
                item.get("absolute_error")
            )
        if name == "iota_profile":
            return _finite_float(
                item.get("minimum_iotas_excluding_axis")
            ) - _finite_float(item.get("floor"))
        ...
        out.append(
            {
                ...
                "initial_update_direction": str(
                    row.get("initial_update_direction", "")
                ),
            }
        )
    return out
...
def _quasilinear_model_rows(rule_path: Path, model_path: Path) -> list[dict[str, Any]]:
            ...
            rows.append(
                {
                    ...
                    "mean_abs_relative_error": _finite_float(
                        item.get("holdout_mean_abs_relative_error")
                    ),
    ...
    if isinstance(metrics, dict):
        rows.append(
            {
                "model": str(
                    model_data.get("required_candidate", "spectral_envelope_ridge")
                ),
...
def _landscape_admission_gate(path: Path) -> dict[str, Any]:
    ...
    passed = (
        ...
        and bool(selected.get("admitted", False))
        and not blockers
    )
    return {
...
def _campaign_admission_gate(path: Path) -> dict[str, Any]:
    ...
    if isinstance(gates, list):
        for row in gates:
            if (
                isinstance(row, dict)
                and row.get("metric") == "reduced_objective_sample_coverage"
            ):
...
def _nonlinear_audit_summary(data: dict[str, Any]) -> dict[str, Any]:
    ...
    uncertainty_separation_sigma = _finite_float(
        comparison.get("uncertainty_separation_sigma")
    )
    metrics_finite = all(
        math.isfinite(value)
        for value in (
...
def build_payload(
    *,
    ...
    projected_relative_change = (
        ...
        if math.isfinite(baseline_metric)
        and baseline_metric != 0.0
        and math.isfinite(projected_metric)
        else math.nan
    )
    ...
    direct_scalar_transport_gate_passed = bool(
        direct_transport_candidate["passed_solved_wout_gate"]
    )
    ...
    projected_transport_improved = bool(
        math.isfinite(projected_relative_change) and projected_relative_change < 0.0
    )
    quasilinear_model_selection_passed = any(
        row["model"] == "spectral_envelope_ridge" and bool(row["passed"])
        for row in ql_rows
    )
    simple_quasilinear_absolute_flux_promoted = any(
        row["source"] == "simple_saturation_rule" and bool(row["passed"])
        for row in ql_rows
    )
    long_window_nonlinear_audit_passed = bool(nonlinear_audit["passed"])
    nonlinear_prelaunch_policy_ready = all(
        bool(row["passed"]) for row in prelaunch_gates
    )
...
def _annotate_status(
    ax: plt.Axes, x: float, y: float, text: str, *, passed: bool
) -> None:
...
def plot_payload(payload: dict[str, Any], out: Path) -> None:
    ...
    metric_values = [
        baseline["transport_metric_final"],
        projected["transport_metric_final"],
    ]
    bars = ax.bar(
        np.arange(2),
        metric_values,
    ...
    ql_values = np.asarray(
        [row["mean_abs_relative_error"] for row in ql_rows], dtype=float
    )
    ...
    ax.axhline(
        0.35, color="#111827", linestyle=":", linewidth=1.1, label="promotion gate"
    )
    ax.set_yscale("log")
    ...
    statuses = [
        ...
        (
            "direct scalar transport",
            not payload["summary"]["direct_scalar_transport_blocked"],
        ),
        (
            "projected metric improves",
            payload["summary"]["projected_transport_improved"],
        ),
        ("prelaunch policy", payload["summary"]["nonlinear_prelaunch_policy_ready"]),
        (
            "QL model selection",
            payload["summary"]["quasilinear_model_selection_passed"],
        ),
        (
            "simple abs-flux QL",
            payload["summary"]["simple_quasilinear_absolute_flux_promoted"],
        ),
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--direct-transport-dir", type=Path, default=DEFAULT_DIRECT_TRANSPORT_DIR
    )
    parser.add_argument(
        "--projected-baseline-dir", type=Path, default=DEFAULT_PROJECTED_BASELINE_DIR
    )
    parser.add_argument(
        "--projected-step-dir", type=Path, default=DEFAULT_PROJECTED_STEP_DIR
    )
    parser.add_argument(
        "--line-search-json", type=Path, default=DEFAULT_LINE_SEARCH_JSON
    )
    ...
    parser.add_argument(
        "--nonlinear-audit-json", type=Path, default=DEFAULT_NONLINEAR_AUDIT_JSON
    )
    parser.add_argument(
        "--landscape-admission-json",
        type=Path,
        default=DEFAULT_LANDSCAPE_ADMISSION_JSON,
    )
    parser.add_argument(
        "--positive-prelaunch-json", type=Path, default=DEFAULT_POSITIVE_PRELAUNCH_JSON
    )
    parser.add_argument(
        "--negative-prelaunch-json", type=Path, default=DEFAULT_NEGATIVE_PRELAUNCH_JSON
    )
    parser.add_argument(
        "--campaign-admission-json", type=Path, default=DEFAULT_CAMPAIGN_ADMISSION_JSON
    )
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "qa_baseline_gate_passed": payload["summary"][
                    "qa_baseline_gate_passed"
                ],
                "direct_scalar_transport_blocked": payload["summary"][
                ...
                "quasilinear_model_selection_passed": payload["summary"][
                    "quasilinear_model_selection_passed"
                ],
                "long_window_nonlinear_audit_passed": payload["summary"][
                    "long_window_nonlinear_audit_passed"
                ],
                "nonlinear_prelaunch_policy_ready": payload["summary"][
                    "nonlinear_prelaunch_policy_ready"
                ],
                "claim_evidence_level": payload["summary"]["claim_evidence_level"],
                "claim_promotion_blockers": payload["summary"][
