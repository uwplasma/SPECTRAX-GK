...
DEFAULT_DATA_DIR = Path(
    os.environ.get(
        "VMEC_JAX_DATA_DIR", "/Users/rogeriojorge/local/vmec_jax/examples/data"
    )
)
...
def _scalar(
    ds: Dataset, *names: str, default: float | int | bool | None = None
) -> float | int | bool | None:
    for name in names:
...
def _family(
    path: Path, nfp: int | None, ntor: int | None, betatotal: float | None
) -> str:
...
def read_vmec_equilibrium_metadata(
    ...
    row["reference_scale_valid"] = all(
        _positive_finite(row.get(name))
        for name in ("aminor", "rmajor", "aspect", "volume")
    )
...
def build_inventory(
    data_dir: str | Path = DEFAULT_DATA_DIR, *, max_files: int | None = None
) -> dict[str, Any]:
    ...
    rows = sorted(
        rows, key=lambda item: (-float(item["candidate_score"]), str(item["name"]))
    )
    ...
    recommended = [
        ...
        for row in rows
        if row["reference_scale_valid"]
        and float(row["candidate_score"]) >= 3.0
        and int(row["size_bytes"]) <= 1_100_000
    ][:8]
    return {
...
def write_inventory_figure(
    report: dict[str, Any], *, out: str | Path = DEFAULT_OUT
) -> dict[str, str]:
    ...
    color_map = {
        family: color for family, color in zip(families, cycle(palette), strict=False)
    }
    aspect = np.asarray(
        [np.nan if row["aspect"] in (None, 0.0) else row["aspect"] for row in rows],
        dtype=float,
    )
    iota_edge = np.asarray(
        [
            np.nan if row["iota_edge"] is None else abs(float(row["iota_edge"]))
            for row in rows
        ]
    )
    sizes = np.asarray([row["size_bytes"] for row in rows], dtype=float)
    marker_size = (
        45.0 + 65.0 * np.clip(np.log10(np.maximum(sizes, 1.0)) - 4.0, 0.0, 2.0) / 2.0
    )
    ...
    for idx, row in enumerate(rows):
        if row["name"] in report["recommended_next_linear_portfolio"][:6]:
            ax0.annotate(
                row["name"].removeprefix("wout_").removesuffix(".nc"),
                (aspect[idx], iota_edge[idx]),
                fontsize=7,
            )
    ...
    fig.suptitle(
        "External VMEC equilibria for future validation, not transport calibration",
        fontsize=14,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Directory containing vmec_jax wout*.nc files.",
    )
    parser.add_argument("--out", default=str(DEFAULT_OUT), help="Output PNG path.")
    parser.add_argument(
        "--max-files", type=int, default=None, help="Optional limit for debugging."
    )
    return parser
