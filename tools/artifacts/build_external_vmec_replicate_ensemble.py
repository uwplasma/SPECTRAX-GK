...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "outputs", nargs="+", type=Path, help="SPECTRAX-GK .out.nc files."
    )
    ...
    parser.add_argument(
        "--artifact-prefix", help="Repo/documentation prefix used in JSON provenance."
    )
...
def _read_heat_flux_trace(
    path: Path, *, heat_flux_variable: str
) -> tuple[np.ndarray, np.ndarray]:
    try:
        import netCDF4
    except Exception as exc:  # pragma: no cover - dependency is in pyproject
        raise RuntimeError(
            "netCDF4 is required to read SPECTRAX-GK output files"
        ) from exc
    ...
    if heat.ndim == 1:
        ...
        raise ValueError(
            f"{path} heat-flux variable has incompatible shape {heat.shape}"
        )
...
def _write_png(
    *,
    ...
    ax0.axvspan(
        float(tmin), float(tmax), color="#d9a441", alpha=0.18, label="accepted window"
    )
    ...
    means = np.asarray(
        [_finite_or_nan(row.get("late_mean")) for row in rows], dtype=float
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for output in args.outputs:
        ...
        time, heat_flux = _read_heat_flux_trace(
            output, heat_flux_variable=str(args.heat_flux_variable)
        )
        _write_trace(trace_path, time, heat_flux)
        summary_artifact = _artifact_path(
            summary_path, out_dir=out_dir, artifact_prefix=args.artifact_prefix
        )
        trace_artifact = _artifact_path(
            trace_path, out_dir=out_dir, artifact_prefix=args.artifact_prefix
        )
        ...
        report_artifact = _artifact_path(
            report_path, out_dir=out_dir, artifact_prefix=args.artifact_prefix
        )
        records.append(
            _record_from_report(
                case=str(args.case),
        ...
        traces.append(
            {"time": time, "heat_flux": heat_flux, "variant": variant["variant"]}
        )
    ...
    _write_png(
        ...
        title=str(
            args.figure_title
            or f"{args.case} nonlinear replicate gate, t=[{args.tmin:g},{args.tmax:g}]"
        ),
        out_png=png_path,
    )
    payload = {
        "readiness_json": _artifact_path(
            readiness_path, out_dir=out_dir, artifact_prefix=args.artifact_prefix
        ),
        "ensemble_json": _artifact_path(
            ensemble_path, out_dir=out_dir, artifact_prefix=args.artifact_prefix
        ),
        "png": _artifact_path(
            png_path, out_dir=out_dir, artifact_prefix=args.artifact_prefix
        ),
