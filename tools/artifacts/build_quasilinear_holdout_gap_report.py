...
from tools.release.check_quasilinear_promotion_guardrails import (  # noqa: E402
...
def _relative_error(
    observed: Any, predicted: Any, *, floor: float = 1.0e-12
) -> float | None:
...
def _load_external_gate(path: Path) -> dict[str, Any]:
    ...
    finite_ratios = [
        ratio for ratio in ratios if ratio is not None and math.isfinite(ratio)
    ]
    failed_ratios = [
        ratio
    ...
    failed_details = _failed_gate_details(
...
def _point_gate_status(
    ...
    *,
    ...
    if input_case:
        ...
        gate_passed = bool(
            input_case.get("passed", input_case.get("gate_passed", False))
        )
    ...
    *,
    ...
    for point in points:
        ...
        if gate_passed is False:
            reason = (
                "listed in train/holdout report but matched input gate is not passed"
            )
        if gate_reason:
            reason = f"{reason}; {gate_reason}"
        row = {
            "section": "admitted_holdouts"
            if split == "holdout"
            else "training_references",
            "rank": None,
            "status": status,
            "case": case,
            ...
            "source_artifact": _repo_relative_path(
                train_holdout.get("artifact", DEFAULT_TRAIN_HOLDOUT)
            ),
            "nonlinear_artifact": _repo_relative_path(point.get("nonlinear_artifact")),
            "quasilinear_artifact": _repo_relative_path(
                point.get("quasilinear_artifact")
            ),
        ...
        rows.append(
            {
                ...
                "gate_case": case,
                "gate_passed": bool(
                    item.get("gate_passed", window_passed.get(case, False))
                ),
                ...
                "reason": str(
                    item.get(
                        "reason",
                        "validated nonlinear case is outside this electrostatic QL lane",
                    )
                ),
                "absolute_flux_promoted": False,
...
def _window_excluded_rows(
    ...
    *,
    ...
    for raw_case in cases:
        ...
        status = (
            "excluded_not_in_quasilinear_dataset"
            if gate_passed
            else "excluded_failed_window_gate"
        )
        if gate_passed:
    ...
    *,
    ...
    for gate in external_gates:
        ...
        if passed and family in holdout_families:
            ...
            detail = "; ".join(failed_details) or str(
                gate.get("reason", "failed external-VMEC gate")
            )
            reason = (
                f"not admitted because external-VMEC convergence gate failed: {detail}"
            )
...
def _next_candidates(
    ...
    *,
    ...
    for row in excluded_rows:
        ...
        candidates.append(
            ((priority, score, str(candidate.get("case", ""))), candidate)
        )
    sorted_candidates = [
        candidate for _, candidate in sorted(candidates, key=lambda item: item[0])
    ]
    ...
    if train_candidate is not None:
        ...
        if failed_candidate is not None:
            detail["nearest_tracked_gap"] = {
                ...
                "failed_gates": failed_candidate.get(
                    "failed_gate_details", failed_candidate.get("failed_gates", [])
                ),
    ...
    for source, role in (
        ...
        (
            [
                row
                for row in excluded_rows
                if _is_nonaxisymmetric_external_vmec_family(
            ...
            rows.append(
                {
                    ...
                    "failed_gates": list(
                        row.get("failed_gate_details", row.get("failed_gates", []))
                    ),
...
def _absolute_flux_promotion_requirements(
    *,
    ...
    admitted_families = {
        str(row.get("geometry", "")) for row in admitted if str(row.get("geometry", ""))
    }
    training_families = {
        str(row.get("geometry", "")) for row in training if str(row.get("geometry", ""))
    }
    ...
    if finite_admitted:
        worst_row = max(
            ...
            key=lambda row: float(
                _finite_float(row.get("absolute_relative_error")) or 0.0
            ),
        )
        worst = {
            "case": worst_row.get("case"),
            "family": worst_row.get("geometry"),
            "absolute_relative_error": _finite_float(
                worst_row.get("absolute_relative_error")
            ),
...
def _model_selection_summary(model_selection: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "candidate_prediction_interval_coverage": _finite_float(
            metrics.get("candidate_prediction_interval_coverage")
        ),
        "transport_mean_relative_error_gate": _finite_float(
            metrics.get("transport_mean_relative_error_gate")
        ),
        "absolute_flux_promoted": False,
...
def _calibration_summary(train_holdout: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(by_split, dict):
        by_split = {}
    holdout = (
        by_split.get("holdout", {})
        if isinstance(by_split.get("holdout", {}), dict)
        else {}
    )
    train = (
        by_split.get("train", {}) if isinstance(by_split.get("train", {}), dict) else {}
    )
    return {
        ...
        "holdout_max_abs_relative_error": _finite_float(
            holdout.get("max_abs_relative_error")
        ),
        "holdout_mean_rel_gate": _finite_float(
            train_holdout.get("holdout_mean_rel_gate")
        ),
        "absolute_flux_promoted": False,
...
def _screening_skill_summary(
    screening_skill: dict[str, Any] | None,
) -> dict[str, Any] | None:
    ...
    return {
        ...
        "accepted_screening_models": list(
            gates.get("accepted_screening_models", []) or []
        ),
        "accepted_holdout_screening_models": list(
            gates.get("accepted_holdout_screening_models", []) or []
        ),
        "accepted_absolute_flux_models": list(
            gates.get("accepted_absolute_flux_models", []) or []
        ),
        "screening_correlation_passed": bool(
            gates.get("screening_correlation_passed", False)
        ),
        "holdout_screening_correlation_passed": bool(
            gates.get("holdout_screening_correlation_passed", False)
        ),
        "absolute_flux_promotion_passed": bool(
            gates.get("absolute_flux_promotion_passed", False)
        ),
        ...
        "best_holdout_spearman": _finite_float(
            best_holdout_row.get("holdout_spearman")
        ),
...
def _screening_promotion_requirements(
    ...
    *,
    ...
    if not isinstance(coverage_gap, dict):
        coverage_gap = {}
    additional_holdouts = coverage_gap.get(
        "additional_total_independent_holdouts_needed"
    )
    gates = [
        {
        ...
        },
        {
            ...
            "passed": bool(
                screening.get("holdout_screening_correlation_passed", False)
            ),
    ...
    blockers = [
        str(gate["metric"])
        for gate in gates
        if gate["metric"] != "absolute_flux_promotion_passed"
    ...
    return {
        ...
        "current_best_holdout_metrics": {
            ...
            "pairwise_order_accuracy": screening.get(
                "best_holdout_pairwise_order_accuracy"
            ),
            "mean_abs_relative_error": screening.get(
                "best_holdout_mean_abs_relative_error"
            ),
        },
    }
...
def build_holdout_gap_report(
    *,
    ...
    external_passed = sum(
        1 for gate in external_gates if bool(gate.get("passed", False))
    )
    ...
    if (
        calibration.get("holdout_mean_abs_relative_error") is not None
        and calibration.get("holdout_mean_rel_gate") is not None
    ):
        if float(calibration["holdout_mean_abs_relative_error"]) > float(
            calibration["holdout_mean_rel_gate"]
        ):
    ...
    payload = {
        ...
        "inputs": {
            ...
            "dataset_sufficiency": _repo_relative_path(DEFAULT_DATASET_SUFFICIENCY)
            if dataset
            else None,
            "screening_skill": _repo_relative_path(DEFAULT_SCREENING_SKILL)
            if screening
            else None,
            "external_gate_patterns": [
                _repo_relative_pattern(pattern) for pattern in external_patterns or []
            ],
        },
        ...
        "summary": {
            ...
            "n_nonlinear_window_cases_passed": sum(
                1 for passed in window_passed.values() if passed
            ),
            "holdout_mean_abs_relative_error": calibration.get(
                "holdout_mean_abs_relative_error"
            ),
            "holdout_mean_rel_gate": calibration.get("holdout_mean_rel_gate"),
            "model_selection_candidate_mean_abs_relative_error": model.get(
                "candidate_mean_abs_relative_error"
            ),
            "model_selection_interval_coverage": model.get(
                "candidate_prediction_interval_coverage"
            ),
            "screening_correlation_passed": None
            if screening is None
            else screening.get("screening_correlation_passed"),
...
def holdout_gap_figure(report: dict[str, Any], *, title: str) -> plt.Figure:
    ...
    admitted = [
        row for row in report.get("admitted_holdouts", []) if isinstance(row, dict)
    ]
    ...
    if values:
        ...
        gate = _finite_float(
            (report.get("calibration_status") or {}).get("holdout_mean_rel_gate")
        )
        if gate is not None:
            ax_error.axvline(
                gate,
    ...
    external_rows = [
        ...
        and str(row.get("geometry", "")).endswith("external_vmec")
    ]
    ...
    ext_labels = [
        _short_label(str(row.get("case", "")), max_chars=52) for row in ranked_external
    ]
    ext_scores = [
        0.0
        if row.get("gate_passed") is True
        else float(_finite_float(row.get("next_best_score")) or 0.0)
        for row in ranked_external
    ]
    if ranked_external:
        ypos = np.arange(len(ranked_external))
        colors = [
            "#2a9d8f" if row.get("gate_passed") is True else "#d1495b"
            for row in ranked_external
        ]
        ...
        for pos, row, score in zip(ypos, ranked_external, ext_scores, strict=True):
            ...
            ax_external.text(
                max(score, 0.03) + 0.04, pos, label, va="center", fontsize=8
            )
    else:
    ...
    model = (
        report.get("model_selection_status", {})
        if isinstance(report.get("model_selection_status", {}), dict)
        else {}
    )
    screening = report.get("screening_skill_status", {})
    screening = screening if isinstance(screening, dict) else {}
    calibration = (
        report.get("calibration_status", {})
        if isinstance(report.get("calibration_status", {}), dict)
        else {}
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--nonlinear-window-statistics", type=Path, default=DEFAULT_WINDOW_STATS
    )
    ...
    parser.add_argument(
        "--max-next-candidates", type=int, default=DEFAULT_NEXT_CANDIDATES
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    dataset = (
        _load_json(args.dataset_sufficiency)
        if args.dataset_sufficiency.exists()
        else None
    )
    ...
    report = build_holdout_gap_report(
        ...
        dataset=dataset,
        screening_skill=_load_json(args.screening_skill)
        if args.screening_skill.exists()
        else None,
