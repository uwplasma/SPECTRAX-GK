...
def _best_recurrence_candidate(payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    finite_rows = [
        ...
        and _finite_float(row.get("mean_abs_error")) is not None
    ]
    if not finite_rows:
        return {
            "label": None,
...
def _best_hypercollision_probe(payload: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    return {
        ...
        "tail_std_ratio": ratio,
        "hermite_tail": _finite_float(best.get("hermite_tail_at_tmax")),
        "free_energy_ratio": _finite_float(
...
def build_status_payload(root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    high_moment_mixed_lm_resolution = _highest_moment_candidate(
    ...
    geom_inverse_res = (
        _finite_float(geom_inverse.get("final_residual_norm"))
    ...
    geom_vmec_equal_arc_core_scalar = _finite_float(
    ...
    geom_vmec_state_abs = _finite_float(
        geom_vmec_state_sensitivity.get("max_abs_ad_fd_error")
    )
    geom_vmec_state_rel = _finite_float(
    ...
    if isinstance(geom_matrix_rows, list):
        finite_drift_rows = [
            ...
            if isinstance(row, dict)
            and _finite_float(row.get("equal_arc_drift_worst_normalized_max_abs"))
            is not None
        ]
