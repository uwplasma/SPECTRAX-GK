...
def build_report(
    *,
    ...
    for model in DEFAULT_MODELS:
        ...
        pairwise_accuracy, pairwise_correct, pairwise_total = _pairwise_order_accuracy(
            predicted, observed
        )
        holdout_pairwise_accuracy, holdout_correct, holdout_total = (
            _pairwise_order_accuracy(predicted[holdout_mask], observed[holdout_mask])
        )
        spearman = _spearman(predicted, observed)
        holdout_spearman = _spearman(predicted[holdout_mask], observed[holdout_mask])
        log_pearson = _pearson(
...
def write_figure(
    ...
    ax0.scatter(
        ...
        s=54,
        color=MODEL_COLORS["linear_weight"],
        edgecolor="white",
    ...
    ax0.scatter(
        ...
        s=65,
        color=MODEL_COLORS["positive_mixing_length"],
        marker="x",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out", type=Path, default=STATIC / "quasilinear_screening_skill.png"
    )
    parser.add_argument(
