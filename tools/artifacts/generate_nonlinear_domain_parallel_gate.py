...
DEFAULT_OUT_JSON = (
    REPO_ROOT / "docs" / "_static" / "nonlinear_domain_parallel_identity_gate.json"
)
DEFAULT_OUT_PNG = (
    REPO_ROOT / "docs" / "_static" / "nonlinear_domain_parallel_identity_gate.png"
)
...
def build_nonlinear_domain_parallel_gate(
    *,
    ...
    serial_axis_trace = np.asarray(
        jnp.real(serial[:, 0] if plan.axis == 0 else serial[0, :])
    )
    ...
    return _json_clean(
        {
                ...
                "metrics": [
                    {
                        ...
                        "identity_passed": bool(
                            transport_report.mass_trace_max_abs_error
                            <= transport_report.atol
                            and transport_report.mass_trace_max_rel_error
            ...
            "gated_state_matches_decomposed": bool(
                np.allclose(
                    np.asarray(gated_state),
                    np.asarray(decomposed),
                    atol=atol,
                    rtol=rtol,
                )
            ),
            "gated_state_matches_serial": bool(
                np.allclose(
                    np.asarray(gated_state), np.asarray(serial), atol=atol, rtol=rtol
                )
            ),
            "rows": rows,
        }
