...
DEFAULT_BASELINE_INPUT = (
    ROOT / "tools_out/latest_vmec_stack/authoritative_qa_baseline/input.final"
)
...
DEFAULT_FRACTIONS = tuple(
    float(round(value, 2)) for value in np.linspace(-0.75, 0.75, 31)
)
DEFAULT_KINDS = (
    "growth",
...
def _coefficient_slug(spec: CoefficientSpec) -> str:
    return f"{spec.family}_{spec.m}_{spec.n}".replace("-", "m").replace("+", "").lower()
...
def _write_scan_inputs(
    *,
    ...
    if abs(base_value) <= 0.0:
        ...
        scan_amplitude, scan_reference = max(
            reference_values, key=lambda item: abs(item[0])
        )
        ...
        rows.append(
            {
                ...
                "expected_wout": input_dir
                / f"wout_{_coefficient_slug(coefficient)}_{label}.nc",
            }
        )
...
def _matching_float_lists(
    left: tuple[float, ...], right: list[Any], *, rtol: float = 1.0e-10
) -> bool:
    if len(left) != len(right):
        return False
    return all(
        math.isclose(float(a), float(b), rel_tol=rtol, abs_tol=rtol)
...
def _reuse_reduced_metrics_from_report(
    *,
    ...
    if not math.isclose(
        old_base, float(baseline_value), rel_tol=1.0e-10, abs_tol=1.0e-12
    ):
        raise ValueError(
    ...
    by_label = {
        str(row.get("label")): row for row in reusable_rows if isinstance(row, dict)
    }
    for row in rows:
        stored = by_label.get(str(row["label"]))
        if stored is None:
            raise ValueError(
                f"{path} has no reusable reduced metrics for label {row['label']!r}"
            )
        old_value = float(stored.get("coefficient_value"))
        if not math.isclose(
            old_value, float(row["coefficient_value"]), rel_tol=1.0e-10, abs_tol=1.0e-12
        ):
            raise ValueError(
        ...
        for kind in kinds:
            ...
            payload = (
                stored_report.get("payload", {})
                if isinstance(stored_report, dict)
                else {}
            )
            sample_statistics = (
                payload.get("sample_statistics") if isinstance(payload, dict) else None
            )
...
def _normalization_scale(values: np.ndarray, *, baseline_index: int) -> float:
    ...
    scale = (
        float(values[baseline_index])
        if np.isfinite(values[baseline_index])
        else float("nan")
    )
...
def _write_plot(
    *,
    ...
    set_plot_style()
    x = (
        np.asarray([float(row["relative_fraction"]) for row in rows], dtype=float)
        * 100.0
    )
    baseline_value = float(report["baseline_coefficient_value"])
    scan_amplitude = float(report.get("scan_amplitude", baseline_value))
    metric_kinds = tuple(
        str(kind) for kind in report.get("reduced_metric_kinds", DEFAULT_KINDS)
    )
    metrics = {
        kind: np.asarray(
            [row.get("reduced_metrics", {}).get(kind, np.nan) for row in rows],
            dtype=float,
        )
    ...
    fig, axes = plt.subplots(
        2, 1, figsize=(7.8, 6.6), sharex=True, constrained_layout=True
    )
    ...
    top_kinds = tuple(
        kind
        for kind in metric_kinds
        if kind == "growth" or kind.startswith("quasilinear_flux")
    )
    ...
    if (
        positive.size
        and float(np.nanmax(positive) / max(np.nanmin(positive), 1.0e-300)) > 100.0
    ):
        ax0.set_yscale("log")
    ...
    if nonlinear_points:
        ...
        xs = np.asarray(
            [
                (float(point["coefficient_value"]) / base - 1.0) * 100.0
        ...
        colors_nl = [
            "#0f766e" if bool(point.get("passed", False)) else "#b91c1c"
            for point in nonlinear_points
        ]
        order = np.argsort(xs)
        ax1.errorbar(
        ...
        ax1.scatter(
            xs, means, c=colors_nl, s=48, edgecolors="0.15", linewidths=0.5, zorder=3
        )
    else:
        ax1.text(
            0.5,
    ...
    ax_secondary = ax1.secondary_xaxis(
        "top",
        functions=(
            lambda frac_percent: baseline_value
            + scan_amplitude * (frac_percent / 100.0),
    ...
    *,
    ...
    return {
        ...
        "postprocess_instruction": (
            ...
            "tools/artifacts/build_external_vmec_replicate_ensemble.py --allow-failed-gates so failed "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--fractions", type=_parse_float_list, default=DEFAULT_FRACTIONS
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    zero_reference_coefficients = tuple(
        _parse_coefficient_spec(str(item))
        for item in (args.zero_reference_coefficient or [])
    )
    ...
    for row in rows:
        if bool(args.evaluate_reduced):
            missing_kinds = tuple(
                kind for kind in kinds if kind not in row["reduced_metrics"]
            )
            ...
            for kind in missing_kinds:
                result = _run_reduced_metric(
                    row=row, kind=kind, args=args, metric_dir=metric_dir
                )
                row["reduced_metric_reports"][kind] = result
                if result.get("value") is not None:
                    row["reduced_metrics"][kind] = float(result["value"])
    nonlinear_points = [
        _load_nonlinear_ensemble(raw) for raw in args.nonlinear_ensemble
    ]
    ...
    report = {
        ...
        "scan_amplitude": scan_amplitude,
        "scan_reference": scan_reference,
        "scan_mode": "relative_to_baseline"
        if abs(base_value) > 0.0
        else "absolute_from_reference",
    ...
    launch_path.write_text(
        json.dumps(
            _json_clean(report["nonlinear_launch_manifest"]), indent=2, sort_keys=True
        )
        + "\n",
        encoding="utf-8",
    )
