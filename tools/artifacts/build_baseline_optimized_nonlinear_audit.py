...
DEFAULT_SELECTED_OPTIMIZED_AUDIT = (
...
def _ensemble_row(
    path: Path, payload: Mapping[str, Any] | None, *, role: str
) -> dict[str, Any]:
    if payload is None:
        return {
            "role": role,
...
def _selected_audit_row(
    ...
    *,
    ...
    promotion_passed = isinstance(promotion_gate, Mapping) and bool(
        promotion_gate.get("passed", False)
    )
    ...
    *,
    ...
    if baseline_sem is not None and optimized_sem is not None:
        ...
        if comparison["absolute_delta"] is not None and combined_uncertainty > 0.0:
            comparison["uncertainty_separation_sigma"] = (
                abs(float(comparison["absolute_delta"])) / combined_uncertainty
            )
    ...
    reduction_passed = relative_reduction is not None and relative_reduction >= float(
    ...
    gates = [
        ...
        },
        {
            ...
            "detail": "; ".join(str(item) for item in baseline.get("blockers", []))
            or "qualified",
        },
        {
            "metric": "optimized_replicated_ensemble_qualified",
            "passed": bool(optimized.get("qualifies", False)),
            "detail": "; ".join(str(item) for item in optimized.get("blockers", []))
            or "qualified",
        },
        {
            "metric": "selected_optimized_equilibrium_audit",
            "passed": bool(selected_audit.get("passed", False))
            and bool(selected_audit.get("optimized_ensemble_selected", False)),
            "detail": "; ".join(
                str(item) for item in selected_audit.get("blockers", [])
            )
            or "selected optimized audit closed",
        },
        {
...
def _write_plot(report: Mapping[str, Any], path: Path) -> None:
    ...
    colors = (
        ["#2563eb", "#0f766e"] if bool(report["passed"]) else ["#6b7280", "#b91c1c"]
    )
    fig, ax = plt.subplots(figsize=(6.4, 4.6))
    ax.bar(
        labels, means, yerr=sems, capsize=5, color=colors, alpha=0.88, edgecolor="0.2"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--baseline-ensemble", type=Path, default=DEFAULT_BASELINE_ENSEMBLE
    )
    parser.add_argument(
        "--optimized-ensemble", type=Path, default=DEFAULT_OPTIMIZED_ENSEMBLE
    )
    parser.add_argument(
        "--selected-optimized-audit",
        type=Path,
        default=DEFAULT_SELECTED_OPTIMIZED_AUDIT,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--case", default="baseline_to_optimized_nonlinear_transport_audit"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    selected_path = (
        None if args.no_selected_optimized_audit else args.selected_optimized_audit
    )
    report = build_audit(
