...
def _relative_error(
    predicted: float | None, observed: float | None, *, floor: float = 1.0e-12
) -> float | None:
...
def build_report(
    *,
    ...
    for idx, case in enumerate(sat["cases"]):
        ...
        row["spectral_envelope_ridge_relative_error"] = _relative_error(
            predicted, observed
        )
...
def write_figure(
    ...
    ax1.bar(
        ...
        spectral,
        width,
        color=MODEL_COLORS["spectral_envelope_ridge"],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out", type=Path, default=STATIC / "quasilinear_stellarator_usefulness.png"
    )
    parser.add_argument(
