#!/usr/bin/env python3
"""Build the VMEC/Boozer-state reduced nonlinear-window gradient artifact."""
...
from spectraxgk.solver_vmec_boozer_gradient_gates import mode21_vmec_boozer_nonlinear_window_gradient_report  # noqa: E402
from tools.build_solver_objective_gradient_gate import _json_clean, write_solver_objective_gradient_artifacts  # noqa: E402

DEFAULT_OUT = ROOT / "docs" / "_static" / "vmec_boozer_nonlinear_window_gradient_gate.png"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--rtol", type=float, default=7.5e-2)
    ...
    parser.add_argument("--nonlinear-dt", type=float, default=0.18)
    parser.add_argument("--nonlinear-steps", type=int, default=96)
    parser.add_argument("--tail-fraction", type=float, default=0.30)
    parser.add_argument(
        "--surface-stencil-width",
        type=int,
        default=0,
        help="Boozer radial stencil width; 0 transforms all radial surfaces, positive values must be >= 3.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if 0 < args.surface_stencil_width < 3:
        raise SystemExit("--surface-stencil-width must be 0 or at least 3")
    payload = mode21_vmec_boozer_nonlinear_window_gradient_report(
