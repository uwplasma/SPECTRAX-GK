...
def load_spectrum(path: str | Path) -> dict[str, np.ndarray]:
    ...
    return {key: np.asarray([row[key] for row in rows], dtype=float) for key in ("ky", "gamma", "omega")}
...
def build_qi_branch_refinement_report(
    ...
    *,
    ...
    subgates: dict[str, dict[str, Any]] = {
        "finite_rows": {"passed": bool(ky.size >= int(min_points)), "value": int(ky.size), "threshold": int(min_points)},
        "positive_run": {
...
def write_gate_figure(
    ...
    *,
    ...
    ax.axhline(float(report["min_launch_gamma"]), color="#b44a3c", linestyle="--", label="launch threshold")
    ...
    lines.extend(["", "Claim boundary:", "linear branch refinement only;", "no nonlinear transport validation."])
    ax_text.axis("off")
...
def main(argv: list[str] | None = None) -> int:
    ...
    outputs = write_gate_figure(spectrum, report, out=args.out, dpi=int(args.dpi), write_pdf=not args.no_pdf)
    report = dict(report)
    ...
    print(json.dumps({"passed": report["passed"], "json": str(json_path), **outputs}, indent=2, sort_keys=True))
