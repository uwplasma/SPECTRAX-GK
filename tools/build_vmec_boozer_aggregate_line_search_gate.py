...
def write_vmec_boozer_aggregate_line_search_artifacts(
    ...
    *,
    ...
    step_labels = [str(row.get("step", index)) for index, row in enumerate(rows) if isinstance(row, dict)]
    objectives = np.asarray(
        [float(row.get("objective", np.nan)) for row in rows if isinstance(row, dict)],
        dtype=float,
    )
    candidates = np.asarray(
        [float(row.get("candidate_objective", np.nan)) for row in rows if isinstance(row, dict)],
        dtype=float,
    )
    curvature = np.asarray(
        [float(row.get("curvature_ratio", np.nan)) for row in rows if isinstance(row, dict)],
        dtype=float,
    )

    set_plot_style()
    fig, (ax_obj, ax_meta) = plt.subplots(1, 2, figsize=(12.0, 5.0), gridspec_kw={"width_ratios": [1.25, 1.0]})
    x = np.arange(max(len(objectives), 1))
    if objectives.size:
        ax_obj.plot(x, objectives, marker="o", lw=2.0, color="#005f73", label="current")
    if candidates.size:
        ax_obj.plot(x, candidates, marker="s", lw=1.8, color="#ca6702", label="candidate")
    ...
    if curvature.size and np.all(np.isfinite(curvature)):
        ...
        ax_curve.plot(x, curvature, marker="^", lw=1.3, ls="--", color="#6a4c93", label="curvature")
