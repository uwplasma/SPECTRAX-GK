...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("files", nargs="*", type=Path, help="Optional explicit test files.")
    parser.add_argument("--test-dir", type=Path, default=DEFAULT_TEST_DIR, help="Directory containing test_*.py files.")
    parser.add_argument("--timeout", type=float, default=300.0, help="Per-file pytest timeout in seconds.")
    parser.add_argument(
        "--total-timeout",
        type=float,
        default=300.0,
...
def run_tests(
    ...
    *,
    ...
    for path in files:
        ...
        t0 = time.monotonic()
        cmd = [sys.executable, "-m", "pytest", "-q", "--maxfail=1", "--disable-warnings", *pytest_args, str(path)]
        try:
