...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("ensembles", nargs="+", type=Path, help="Nonlinear ensemble JSON files.")
...
def _enrich_ensemble(payload: dict[str, Any], *, ensemble_path: Path) -> dict[str, Any]:
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        summary_path = _first_existing(_candidate_paths(row.get("summary_artifact"), ensemble_path=ensemble_path))
...
def _write_png(report: dict[str, Any], out_png: Path) -> None:
    ...
    for state_index, state in enumerate(states):
        ...
        for local_index, row in enumerate(group):
            ...
            offset = offsets.get(axis, 0.0) + 0.06 * (local_index - (len(group) - 1) / 2)
        ...
        if state_summary.get("classification") != "passed_replicate_spread_gate":
            ax.axvspan(state_index - 0.45, state_index + 0.45, color="#fee2e2", alpha=0.35, lw=0)
