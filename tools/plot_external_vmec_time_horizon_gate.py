...
def build_time_horizon_payload(
    ...
    *,
    ...
    for horizon, path in sorted(entries, key=lambda item: item[0]):
        ...
        common_means = [float(run["common_window"]["heat_flux_mean"]) for run in gate["runs"]]
        least_means = [float(run["least_trending_window"]["heat_flux_mean"]) for run in gate["runs"]]
        rows.append(
            {
                ...
                "grid_common_relative_difference": float(
                    gate["common_window"]["max_pairwise_heat_flux_symmetric_relative_difference"]
                ),
                "grid_least_relative_difference": float(
                    gate["least_windows"]["max_pairwise_heat_flux_symmetric_relative_difference"]
                ),
                "common_heat_flux_mean": float(np.mean(common_means)),
                "common_heat_flux_half_spread": float(0.5 * (max(common_means) - min(common_means))),
                "least_heat_flux_mean": float(np.mean(least_means)),
                "least_heat_flux_half_spread": float(0.5 * (max(least_means) - min(least_means))),
    ...
    common_change = _max_symmetric_relative_difference([float(row["common_heat_flux_mean"]) for row in rows])
    least_change = _max_symmetric_relative_difference([float(row["least_heat_flux_mean"]) for row in rows])
...
def write_time_horizon_panel(
    ...
    *,
    ...
    payload = build_time_horizon_payload(entries, case=case, max_relative_change=max_relative_change)
    out_path = Path(out)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    horizons = np.asarray([float(row["horizon"]) for row in payload["rows"]], dtype=float)
    common = np.asarray([float(row["common_heat_flux_mean"]) for row in payload["rows"]], dtype=float)
    common_err = np.asarray([float(row["common_heat_flux_half_spread"]) for row in payload["rows"]], dtype=float)
    least = np.asarray([float(row["least_heat_flux_mean"]) for row in payload["rows"]], dtype=float)
    least_err = np.asarray([float(row["least_heat_flux_half_spread"]) for row in payload["rows"]], dtype=float)

    set_plot_style()
    fig, axes = plt.subplots(1, 2, figsize=(12.0, 4.8), constrained_layout=True)
    ax, ax_text = axes
    ax.errorbar(horizons, common, yerr=common_err, marker="o", linewidth=2.2, capsize=4, label="common window")
    ax.errorbar(horizons, least, yerr=least_err, marker="s", linewidth=2.2, capsize=4, label="least-trending window")
    ...
    failed = [item for item in payload["gate_report"]["gates"] if not bool(item["passed"])]
    lines = [
    ...
    lines.extend(f"- {item['metric']}: {float(item['observed']):.3g} > {float(item['atol']):.3g}" for item in failed)
    if not failed:
        lines.append("- none")
    lines.extend(
        [
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--entry", action="append", required=True, help="Horizon/gate entry encoded as HORIZON:PATH")
    parser.add_argument("--out", default=str(DEFAULT_OUT), help="Output PNG path.")
    parser.add_argument("--case", default="External-VMEC high-grid time-horizon stability")
    parser.add_argument("--max-relative-change", type=float, default=DEFAULT_MAX_RELATIVE_CHANGE)
    return parser
