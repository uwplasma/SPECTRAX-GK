#!/usr/bin/env python3
"""Generate a full linear-RHS streaming-only identity gate."""
...
DEFAULT_PREFIX = REPO_ROOT / "docs" / "_static" / "linear_rhs_streaming_gate"
...
def build_problem(
    *,
    ...
    """Build a small periodic linear-RHS problem with no field response."""
...
def build_linear_rhs_streaming_gate(
    *,
    ...
    """Compare streaming-only ``linear_rhs_cached`` with the shard-map microkernel."""
...
def write_artifacts(summary: dict[str, object], out_prefix: Path) -> dict[str, str]:
    ...
    axes[0].plot(m, production, "s-", lw=1.8, label="linear_rhs streaming")
