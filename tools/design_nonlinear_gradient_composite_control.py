...
DEFAULT_OUT_PREFIX = ROOT / "docs" / "_static" / "nonlinear_gradient_composite_control_design"
...
def _write_csv(path: Path, report: dict[str, Any]) -> None:
    ...
    weights = {row["source_label"]: row for row in report.get("controls", []) if isinstance(row, dict)}
            ...
            writer.writerow(
                {
                    ...
                    "admissible_for_composite_direction": row.get("admissible_for_composite_direction"),
...
def _plot(path: Path, report: dict[str, Any]) -> None:
    ...
    gradients = np.asarray([_float_or_nan(row.get("metrics", {}).get("central_gradient")) for row in rows])
    descent = np.asarray([_float_or_nan(row.get("metrics", {}).get("descent_gradient")) for row in rows])
    ...
    weights_by_label = {row["source_label"]: row for row in report.get("controls", []) if isinstance(row, dict)}
    weights = np.asarray([_float_or_nan(weights_by_label.get(row.get("label"), {}).get("weight")) for row in rows])
    admissible = np.asarray([bool(row.get("admissible_for_composite_direction")) for row in rows])
    cfg = report["config"]

    set_plot_style()
    fig, axes = plt.subplots(1, 3, figsize=(13.5, 4.0), constrained_layout=True)
    colors = np.where(admissible, "#4daf4a", "#bdbdbd")
    axes[0].bar(x - 0.18, gradients, width=0.36, color="#377eb8", label="central gradient")
    ...
    axes[1].axhline(float(cfg["max_fd_asymmetry_rel"]), color="#ff7f00", ls="--", lw=1.2)
    axes[1].axhline(float(cfg["max_gradient_uncertainty_rel"]), color="#984ea3", ls="--", lw=1.2)
