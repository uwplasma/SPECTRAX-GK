#!/usr/bin/env python3
"""Combine independent-ky CPU/GPU scaling artifacts into one panel."""
...
DEFAULT_PREFIX = REPO_ROOT / "docs" / "_static" / "independent_ky_scan_scaling_large"
...
def write_artifacts(summary: dict[str, Any], out_prefix: Path) -> dict[str, str]:
    ...
    for backend in sorted({str(row["backend"]) for row in rows}):
        ...
        gamma_rel = np.asarray([float(row["max_gamma_rel_error"]) for row in subset], dtype=float)
        omega_abs = np.asarray([float(row["max_omega_abs_error"]) for row in subset], dtype=float)
        ...
        axes[2].semilogy(x, np.maximum(gamma_rel, 1.0e-16), marker=marker, lw=2.0, color=color, label=label + r" $\gamma$")
        axes[2].semilogy(
            x + 0.04,
