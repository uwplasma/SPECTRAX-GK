...
SCRIPT = ROOT / "tools" / "check_nonlinear_transport_matrix_portfolio.py"
spec = importlib.util.spec_from_file_location("check_nonlinear_transport_matrix_portfolio", SCRIPT)
...
def test_portfolio_selects_best_passing_broad_matrix(tmp_path: Path) -> None:
    accepted = _matrix_report(tmp_path / "accepted.json", passed=True, mean_reduction=0.025)
    projected = _matrix_report(tmp_path / "projected.json", passed=True, mean_reduction=0.04)
...
def test_portfolio_blocks_missing_or_failed_broad_matrices(tmp_path: Path) -> None:
    ...
    report = mod.build_report(
        matrix_reports={"accepted_qa_ess": failed, "projected_0p001": tmp_path / "missing.json"},
