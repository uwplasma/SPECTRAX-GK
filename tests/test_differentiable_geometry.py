...
def test_observable_gradient_validation_report_passes_with_conditioning_metadata() -> None:
...
def test_observable_gradient_validation_report_fails_strict_json_on_nonfinite_data() -> None:
    ...
    assert any(str(reason).startswith("nonfinite:") for reason in report["failure_reasons"])
    assert report["conditioning"]["finite_ad_jacobian"] is False
    assert report["conditioning_gate"]["passed"] is False
    json.dumps(report, allow_nan=False)


def test_observable_gradient_validation_report_fails_ill_conditioned_synthetic_map() -> None:
