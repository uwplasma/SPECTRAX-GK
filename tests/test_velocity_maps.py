...
def test_velocity_map_config_properties_and_pytree_round_trip():
    cfg = VelocityMapConfig(
        parallel_shift=jnp.asarray(0.2, dtype=jnp.float64),
        parallel_log_scale=jnp.log(1.4),
        perpendicular_log_scale=jnp.log(0.7),
    )

    np.testing.assert_allclose(float(cfg.parallel_scale), 1.4, rtol=1.0e-14, atol=1.0e-14)
    np.testing.assert_allclose(float(cfg.perpendicular_scale), 0.7, rtol=1.0e-14, atol=1.0e-14)

    leaves, treedef = jax.tree_util.tree_flatten(cfg)
    restored = jax.tree_util.tree_unflatten(treedef, leaves)
    np.testing.assert_allclose(float(restored.parallel_shift), 0.2, rtol=1.0e-14, atol=1.0e-14)
    np.testing.assert_allclose(float(restored.parallel_scale), 1.4, rtol=1.0e-14, atol=1.0e-14)
    np.testing.assert_allclose(float(restored.perpendicular_scale), 0.7, rtol=1.0e-14, atol=1.0e-14)


def test_mapped_parallel_operators_pytree_round_trip():
    ops = mapped_parallel_operators(4, VelocityMapConfig(parallel_shift=0.1, parallel_log_scale=0.2))

    leaves, treedef = jax.tree_util.tree_flatten(ops)
    restored = jax.tree_util.tree_unflatten(treedef, leaves)

    np.testing.assert_allclose(np.asarray(restored.multiply), np.asarray(ops.multiply), rtol=0.0, atol=1.0e-14)
    np.testing.assert_allclose(np.asarray(restored.derivative), np.asarray(ops.derivative), rtol=0.0, atol=1.0e-14)
    np.testing.assert_allclose(np.asarray(restored.energy), np.asarray(ops.energy), rtol=0.0, atol=1.0e-14)
    assert sorted(restored.regularization) == sorted(ops.regularization)


def test_invalid_mode_counts_raise():
    with pytest.raises(ValueError, match="n_modes"):
        hermite_multiply_matrix(0)
    with pytest.raises(ValueError, match="n_modes"):
        hermite_derivative_matrix(0)
    with pytest.raises(ValueError, match="n_modes"):
        laguerre_multiply_matrix(0)
    with pytest.raises(ValueError, match="n_modes"):
        ModalGate(cutoff=1.0).values(0)
...
def test_perpendicular_energy_scale_and_gradient():
    ...
    identity_scaled = mapped_perpendicular_energy_matrix(n)
    np.testing.assert_allclose(np.asarray(identity_scaled), np.asarray(base), rtol=0.0, atol=1.0e-14)
...
def test_modal_gate_bounds_application_and_gradients():
    ...
    complex_arr = jnp.ones((8, 2), dtype=jnp.complex128) * (1.0 + 2.0j)
    complex_gated = gate.apply(complex_arr, axis=0)
    assert complex_gated.dtype == jnp.complex128
    np.testing.assert_allclose(
        np.asarray(complex_gated[:, 0]), np.asarray(vals * (1.0 + 2.0j)), rtol=0.0, atol=1.0e-14
    )
...
def test_gate_regularization_single_mode_roughness_is_zero():
    reg = gate_regularization(ModalGate(cutoff=0.5, width=0.25), 1)
    np.testing.assert_allclose(float(reg["gate_roughness"]), 0.0, rtol=0.0, atol=0.0)
