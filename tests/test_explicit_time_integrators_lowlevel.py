...
def test_explicit_time_lowlevel_array_and_maximum_helpers() -> None:
    ...
    assert eti._gradient_ratio_max(np.asarray([2.0]), np.asarray([0.0])) == pytest.approx(1.0e6)
...
def test_instantaneous_growth_rate_step_max_mode_and_invalid_method() -> None:
    ...
    gamma, omega = eti._instantaneous_growth_rate_step(phi_now, phi_prev, 0.5, z_index=0, mask=mask, mode_method="max")
    ...
    with pytest.raises(ValueError, match="mode_method"):
        eti._instantaneous_growth_rate_step(phi_now, phi_prev, 0.5, z_index=0, mask=mask, mode_method="bad")


@pytest.mark.parametrize("method", ["euler", "rk2", "rk3_classic", "rk3", "rk3_heun", "rk4", "sspx3", "k10"])
def test_linear_explicit_step_methods_match_scalar_linear_amplification(monkeypatch, method: str) -> None:
    ...
    G1, fields = eti._linear_explicit_step(G0, _cache(), object(), object(), 0.05, method=method)
