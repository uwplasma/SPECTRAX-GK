...
from support.paths import REPO_ROOT, load_artifact_tool, load_release_tool
...
def test_family_detection_covers_screen_names() -> None:
    ...
    assert (
        external_vmec_family("shaped_tokamak_pressure_reference_nc")
        == "shaped_tokamak_external_vmec"
    )
    assert (
        external_vmec_family("QI_stel_seed_3127_nc", "/vmec/wout_QI_stel_seed_3127.nc")
        == "qi_external_vmec"
    )
    assert (
        external_vmec_family("li383_low_res_nc", "/vmec/wout_li383_low_res.nc")
        == "li383_external_vmec"
    )
    assert external_vmec_family("basic_non_stellsym_nc") == "non_stellsym_external_vmec"
    assert (
        external_vmec_family("cth_like_reference_nc", "/vmec/wout_cth_like.nc")
        == "cth_like_external_vmec"
    )
    assert (
        external_vmec_family("up_down_asymmetric_tokamak_reference_nc")
        == "updown_asym_external_vmec"
    )
...
def test_read_external_holdout_screen_and_rank_runbook(tmp_path: Path) -> None:
    ...
    with screen.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            ["DSHAPE_nc", "/vmec/wout_DSHAPE.nc", 0, 0.47, 0.096, 0.06, "dshape.log"]
        )
        writer.writerow(
            [
        ...
        writer.writerow(
            ["stable_nc", "/vmec/wout_stable.nc", 0, 0.47, -0.01, 0.1, "stable.log"]
        )
    ...
    assert (
        runbook["selected_preferred_family_audit"]["case"] == "ITERModel_reference_nc"
    )
    assert runbook["selected_new_family_candidate"]["case"] == "circular_tokamak_nc"
    assert (
        runbook["selected_new_family_candidate"]["status"]
        == "new_family_holdout_candidate"
    )
    assert runbook["acceptance_gate"]["required_split"] == "holdout"
    assert any(
        "write_external_vmec_holdout_configs.py" in command
        for command in runbook["launch_commands"]
    )
...
def test_runbook_demotes_recent_failed_external_family() -> None:
    ...
    shaped = next(
        row
        for row in runbook["ranked_candidates"]
        if row["case"].startswith("shaped_tokamak")
    )
...
def test_runbook_allows_failed_family_only_with_explicit_modified_protocol() -> None:
    ...
    cth_default = next(
        row
        for row in default["ranked_candidates"]
        if row["family"] == "cth_like_external_vmec"
    )
...
def test_runbook_requires_note_for_modified_failed_family() -> None:
    ...
    try:
        ...
        raise AssertionError(
            "modified failed-family reruns must require a protocol note"
        )
...
def test_runbook_does_not_relaunch_passed_same_family_audit() -> None:
    ...
    assert (
        runbook["ranked_candidates"][0]["status"]
...
def test_runbook_fails_closed_when_no_unstable_candidate_exists() -> None:
    ...
    assert runbook["launch_commands"] == []
    assert (
        runbook["ranked_candidates"][0]["status"] == "screen_rejected_stable_or_failed"
    )
...
def test_tracked_next_holdout_runbook_launches_new_family_not_failed_qh() -> None:
    ...
    path = REPO_ROOT / "docs" / "_static" / "external_vmec_next_holdout_runbook.json"
    ...
    assert all(
        "solovev_reference_holdout" in command for command in payload["launch_commands"]
    )
    assert all(
        "qh" not in command.lower() and "nfp4" not in command.lower()
        for command in payload["launch_commands"]
    )
    qh_rows = [
        row
        for row in payload["ranked_candidates"]
        if row["family"] == "qh_external_vmec"
    ]
    assert all(
        row["status"] != "modified_protocol_failed_family_candidate" for row in qh_rows
    )
...
def test_runbook_tool_writes_modified_protocol_contract(tmp_path: Path) -> None:
    ...
    gap = tmp_path / "gap.json"
    gap.write_text(
        json.dumps(_gap_report_with_failed_cth_like_family()), encoding="utf-8"
    )
    screen = tmp_path / "screen.csv"
    screen.write_text(
        "\n".join(
    ...
    assert payload["passed"] is True
    assert (
        payload["selected_new_family_candidate"]["family"] == "cth_like_external_vmec"
    )
    assert (
        payload["selected_new_family_candidate"]["status"]
        == "modified_protocol_failed_family_candidate"
    )
    assert (
        payload["modified_protocol_note"]
        == "n80 grid and longer post-transient window repair"
    )
