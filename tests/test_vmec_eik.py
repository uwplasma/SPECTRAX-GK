...
def test_generate_runtime_vmec_eik_invokes_internal_generator(
    ...
    monkeypatch.setattr(
        "spectraxgk.vmec_eik.internal_vmec_backend_available", lambda: True
    )
...
def test_generate_runtime_vmec_eik_reuses_default_cache_without_backend(
    ...
    monkeypatch.setattr(
        "spectraxgk.vmec_eik.default_vmec_eik_output_path",
        lambda _request: expected_out,
    )
    monkeypatch.setattr(
        "spectraxgk.vmec_eik.internal_vmec_backend_available", lambda: False
    )
    mock_gen = MagicMock(
