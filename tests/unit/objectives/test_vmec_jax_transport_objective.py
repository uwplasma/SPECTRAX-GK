...
def test_vmec_objective_term_prepares_and_prewarms_backend(monkeypatch) -> None:
    ...
    cfg = VMECJAXTransportObjectiveConfig(
        kind="growth", ntheta=28, mboz=23, nboz=25, surface_chunk_size=1
    )
    objective = VMECJAXSpectraxTransportObjective(config=cfg, name="transport_test")
    ctx = SimpleNamespace(
        static=SimpleNamespace(cfg=SimpleNamespace(nfp=7)), indata="indata", signgs=-1
    )
...
def test_spectrax_transport_objective_tuple_uses_config_and_wout_reference(
    monkeypatch,
) -> None:
    import spectraxgk.objectives.vmec_transport as mod
    ...
    value = callback(
        SimpleNamespace(static="static", indata="indata", signgs=1), "state"
    )
