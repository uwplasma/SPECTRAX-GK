...
from tools.artifacts import generate_linear_rhs_streaming_electrostatic_gate as gate


def test_linear_rhs_streaming_electrostatic_gate_builds_identity_summary(
    monkeypatch,
) -> None:
    class FakePlan:
        def to_dict(self):
...
def test_linear_rhs_streaming_electrostatic_gate_writes_artifacts(
    tmp_path: Path,
) -> None:
    summary = {
        "rows": [
