...
def _load_tool_module():
    path = (
        ...
        / "tools"
        / "artifacts"
        / "generate_nonlinear_sharding_production_gate.py"
    )
    spec = importlib.util.spec_from_file_location(
        "generate_nonlinear_sharding_production_gate", path
    )
...
def test_nonlinear_sharding_production_gate_loads_sweep_rows(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps({"backend": "gpu", "rows": [_row(backend="gpu", speedup=1.4)]}),
        encoding="utf-8",
    )
