...
def _write_status_json(root: Path, relative: str, payload: dict[str, object]) -> None:
    path = root / relative
...
def test_build_status_payload_keeps_open_lanes_scoped(tmp_path: Path) -> None:
    ...
    _write_nonlinear_performance_status_inputs(tmp_path, include_device_metadata=True)
...
def test_strategy_report_exports_public_qa_transport_claim_boundaries(
    ...
    objective_callbacks = {
        "growth": "turb.turbulent_growth_rate",
        "quasilinear_flux": "turb.quasilinear_flux_proxy",
        "nonlinear_window_heat_flux": "turb.nonlinear_heat_flux_proxy",
    }
    for kind, row in boundaries.items():
        ...
        assert "vj.VmecInput.from_file" in text
        assert "opt.least_squares" in text
        assert objective_callbacks[kind] in text
        assert "FixedBoundaryVMEC" not in text
        assert "LeastSquaresProblem" not in text
    assert "QA_nonlinear_ITG_matched_audit.py" in readme
    assert "QA_nonlinear_ITG_transport_matrix.py" in readme
