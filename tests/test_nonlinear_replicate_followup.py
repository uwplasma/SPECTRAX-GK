...
def test_followup_plan_covers_seed_and_timestep_limited_branches() -> None:
    ...
    metadata = [
        ...
        {"state": "plus_delta", "variant_label": "bad_seed", "seed": -1, "timestep": 0.04},
    ]
...
def test_followup_plan_handles_empty_or_unrunnable_reports() -> None:
    assert nonlinear_replicate_followup_plan({"summary": {}}, variant_metadata=[])["passed"] is True

    unrunnable = nonlinear_replicate_followup_plan(
        {
            "summary": {"failed_states": ["baseline"]},
            "state_rows": [{"state": "baseline", "classification": "seed_spread_limited"}],
        },
        variant_metadata=[],
    )

    assert unrunnable["passed"] is True
    assert unrunnable["state_plans"][0]["recommendation"].startswith("No runnable follow-up")
