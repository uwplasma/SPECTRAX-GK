...
def _operator_matrix(fn, shape: tuple[int, ...]) -> np.ndarray:
    ...
    for idx in range(size):
        basis = jnp.zeros(shape, dtype=jnp.float32).reshape(-1).at[idx].set(1.0).reshape(shape)
...
def test_velocity_multiplication_matrices_are_self_adjoint() -> None:
    ...
    np.testing.assert_allclose(hermite_v2, hermite_v @ hermite_v, rtol=1.0e-6, atol=1.0e-6)
