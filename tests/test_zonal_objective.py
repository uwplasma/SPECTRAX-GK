...
def test_zonal_flow_objective_prefers_large_residual_and_low_damping() -> None:
    ...
    np.testing.assert_allclose(np.asarray(rows[..., 0]), 1.0 / np.asarray(residual_strong))


def test_zonal_flow_objective_sensitivity_report_checks_ad_fd_and_conditioning() -> None:
    ...
    assert report["objective_config"]["objective_names"] == list(ZONAL_FLOW_OBJECTIVE_NAMES)
...
def test_zonal_flow_objective_rejects_invalid_contracts() -> None:
    ...
    with pytest.raises(ValueError, match="dimensions"):
        zonal_flow_objective_rows(residual_level=jnp.ones((0, 1, 1)), damping_rate=jnp.ones((0, 1, 1)))
    ...
    with pytest.raises(ValueError, match="finite"):
        zonal_flow_objective_rows(residual_level=jnp.asarray([[[jnp.nan]]]), damping_rate=jnp.asarray([[[0.1]]]))
    with pytest.raises(ValueError, match="strictly positive"):
        zonal_flow_objective_rows(residual_level=jnp.asarray([[[0.0]]]), damping_rate=jnp.asarray([[[0.1]]]))
