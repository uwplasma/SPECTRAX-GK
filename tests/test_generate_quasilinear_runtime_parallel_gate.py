...
def test_quasilinear_runtime_parallel_gate_builds_identity_summary(monkeypatch) -> None:
    ...
    def fake_timed_scan(_cfg, ky_values, *, workers, **_kwargs):  # type: ignore[no-untyped-def]
        ...
        return _scan(np.asarray(ky_values, dtype=float), workers=workers), 4.0 if workers == 1 else 2.0
...
def test_quasilinear_runtime_parallel_gate_writes_artifacts(tmp_path: Path) -> None:
    ...
    assert json.loads(Path(paths["json"]).read_text(encoding="utf-8"))["identity_passed"] is True
    assert "heat_flux_weight_total_abs_error" in Path(paths["csv"]).read_text(encoding="utf-8")
