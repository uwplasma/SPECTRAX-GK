...
from tools import generate_electrostatic_drift_gate as gate
...
def test_electrostatic_drift_gate_builds_identity_summary(monkeypatch) -> None:
    ...
    def fake_rhs(state, *_args, **kwargs):  # type: ignore[no-untyped-def]
        ...
        scale = float(getattr(terms, "mirror", 0.0)) + float(getattr(terms, "curvature", 0.0)) + float(getattr(terms, "gradb", 0.0))
    ...
    assert {row["component"] for row in summary["rows"]} == {"mirror", "curvature_gradb", "total"}


def test_electrostatic_drift_gate_writes_artifacts(tmp_path: Path) -> None:
    summary = {
        "rows": [
            {
                "component": "mirror",
    ...
    out = tmp_path / "electrostatic_drift_gate"
