...
from tools import generate_linear_rhs_streaming_electrostatic_gate as gate


def test_linear_rhs_streaming_electrostatic_gate_builds_identity_summary(monkeypatch) -> None:
    class FakePlan:
        def to_dict(self):
            return {
    ...
    summary = gate.build_linear_rhs_streaming_electrostatic_gate(
        requested_devices=2,
        nx=1,
        ny=2,
        nz=4,
...
def test_linear_rhs_streaming_electrostatic_gate_writes_artifacts(tmp_path: Path) -> None:
    summary = {
        "rows": [
            {
                "m": 0,
    ...
    out = tmp_path / "linear_rhs_streaming_electrostatic_gate"
