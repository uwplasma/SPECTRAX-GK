...
def plot(output: dict):
    ...
    ax.plot(t, Wg, label=r"$W_H$ (free energy on $H$)")
    ax.plot(t, Wphi, label=r"$W_\phi$ (field proxy)")
    ax.plot(t, Wtot, label=r"$W_{\mathrm{tot}}$ (tracked)")
    ax.set_yscale("log")
    ax.set_xlabel("t")
    ...
    if "D_coll" in output:
        ...
        ax2.set_yscale("log" if np.any(D > 0) else "linear")
        ax2.set_xlabel("t")
...
def _hermite_functions(v: np.ndarray, mmax: int):
    ...
    w = np.exp(-0.5 * v**2)
    for m in range(mmax):
        norm = math.sqrt((2.0**m) * math.factorial(m) * math.sqrt(math.pi))
        psi[m] = H[m] * w / norm
    return psi
