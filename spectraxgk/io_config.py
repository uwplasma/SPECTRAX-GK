...
class SpeciesCfg:
    ...
    temperature_eV: Optional[float] = None  # replaces 'vth' as user input (T in eV)
    drift_c: Optional[float] = None         # replaces 'u0' as user input (u/c)
...
class GridCfg:
    ...
    L:    Optional[float] = None          # meters
    L_lambdaD: Optional[float] = None     # dimensionless, multiples of λ_D (electron Debye length)
    debye_species: Optional[Union[int, str]] = None  # which species to use for λ_D (default: first q<0)
...
class PlotCfg:
    nv: int = 257
    vmin_c: Optional[float] = None   # lower bound in units of c
    vmax_c: Optional[float] = None   # upper bound in units of c
...
def _preprocess_toml_expressions(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        skip = (
            not rhs or
            rhs[0] in "\"'" or                # already quoted string
            rhs[0] in "[{" or                 # array/table/dict
            rhs.lower() in ("true", "false")  # boolean
        )
